...
Les touches d’accès rapide peuvent améliorer à la fois l’utilisation et l’accessibilité de votre application Windows en introduisant une méthode intuitive permettant aux utilisateurs de naviguer et d’interagir rapidement avec l’interface utilisateur visible d’une application, par le biais du clavier plutôt qu’à l’aide d’un périphérique de pointage (comme un pointeur tactile ou une souris).
...
Les touches d’accès sont des badges affichés en regard des commandes qui prennent en charge les touches d’accès rapide lorsque l’utilisateur appuie sur la touche Alt. Chaque touche d’accès contient des touches alphanumériques qui activent la commande associée.
...
Certaines commandes, telles que la commande CommandBar, ne prennent pas encore en charge les étendues intégrées de touche d’accès rapide. Par conséquent, vous devrez les intégrer vous-même. L’exemple suivant montre comment prendre en charge la propriété SecondaryCommands de la classe CommandBar grâce aux touches d’accès rapide disponibles une fois qu’une commande parent est appelée (comme le ruban dans Word).
...
## <a name="position-key-tips"></a>Position des touches d’accès
...
## <a name="style-key-tips"></a>Format des touches d’accès
...
Si vous avez besoin de spécifier vos propres styles de touche d’accès, utilisez des ressources d’application, telles que KeyTipFontSize (taille de la police), KeyTipPadding (marge), KeyTipBorderBrush (couleur de bordure) et KeyTipBorderThemeThickness (épaisseur de bordure).
