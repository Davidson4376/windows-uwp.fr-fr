---
title: Appareil de PointOfService de revendication et activer le modèle
description: En savoir plus sur les revendications PointOfService et activer le modèle
ms.date: 06/19/2018
ms.topic: article
...
ms.locfileid: "57639314"
---
# <a name="point-of-service-device-claim-and-enable-model"></a>Périphérique de point de Service de revendication et activer le modèle

## <a name="claiming-for-exclusive-use"></a>Revendication pour un usage exclusif
...
> [!Note]
> L’action de la revendication établit un verrou exclusif sur un appareil, mais ne la placez pas dans un état opérationnel.  Consultez [appareil activer pour les opérations d’e/s](#enable-device-for-io-operations) pour plus d’informations.

### <a name="apis-used-to-claim--release"></a>Les API utilisées pour la revendication / release
...
L’action de la revendication est simplement établit un droits exclusifs à l’appareil, mais ne la placez pas dans un état opérationnel.  Pour pouvoir recevoir des événements ou effectuer des opérations de sortie, vous devez activer l’appareil en utilisant **EnableAsync**.  À l’inverse, vous pouvez appeler **DisableAsync** pour arrêter d’écouter des événements à partir de l’appareil ou l’exécution de sortie.  Vous pouvez également utiliser **IsEnabled** pour déterminer l’état de votre appareil.
...
Affichage de la ligne ¹ ne nécessite pas activer explicitement l’appareil pour les opérations d’e/s.  L’activation est effectuée automatiquement par les PointOfService LineDisplay APIs qui effectuent des e/s.

## <a name="code-sample-claim-and-enable"></a>Exemple de code : de revendication et activer
...
La première étape consiste à créer un gestionnaire d’événements qui répond à la **ReleaseDeviceRequested** événement avec **RetainDevice**.  
...
Puis inscrire le Gestionnaire d’événements en association avec votre appareil demandée
