...
Cet article montre l’utilisation de l’API de Client attribut générique (GATT Bluetooth) pour les applications de plateforme Windows universelle (UWP), ainsi qu’un exemple de code pour les tâches courantes de client GATT:
...
Services définissent le contrat fonctionnel du périphérique LE et contiennent une collection de caractéristiques qui définissent le service. Ces caractéristiques, à leur tour, contiennent des descripteurs qui décrivent les caractéristiques. Ces 3 termes sont appelés les attributs d’un appareil.

API GATT Bluetooth Bluetooth LE exposent des objets et des fonctions, plutôt qu’accéder au transport. Les API GATT permettent également aux développeurs d’utiliser des appareils Bluetooth LE la possibilité d’effectuer les tâches suivantes:
...
deviceWatcher.Start();
```
Une fois que vous avez démarré le DeviceWatcher, vous recevrez [DeviceInformation](https://msdn.microsoft.com/library/windows/apps/br225393) pour chaque appareil qui répond à la requête dans le Gestionnaire de l’événement [ajoutée](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.devicewatcher.added) pour les appareils en question. Pour une présentation plus détaillée à DeviceWatcher reportez-vous à la fin de l’exemple [sur Github](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/DeviceEnumerationAndPairing). 
...
```
En revanche, de toutes les références à un BluetoothLEDevice supprimer objet pour un appareil (et si aucune autre application sur le système ne dispose d’une référence à l’appareil) déclenchera automatique déconnecter après un délai d’expiration petite. 

```csharp
bluetoothLeDevice.Dispose();
```
Si l’application a besoin d’accéder à nouveau l’appareil, il vous suffit recréer l’objet de périphérique et accéder à une caractéristique (abordée dans la section suivante) déclenche le système d’exploitation pour vous reconnecter lorsque cela est nécessaire. Si l’appareil se trouve à proximité, vous obtiendrez l’accès à l’appareil dans le cas contraire, qu'elle renvoie avec une erreur DeviceUnreachable.  

## <a name="enumerating-supported-services-and-characteristics"></a>Énumération des services pris en charge et caractéristiques
...
```
Une fois le service d’intérêt a été identifié, l’étape suivante consiste à rechercher les caractéristiques. 
...
La caractéristique est communication basée sur l’unité fondamentale de GATT. Elle contient une valeur qui représente un élément distinct de données sur l’appareil. Par exemple, la caractéristique de niveau de batterie a une valeur qui représente le niveau de batterie de l’appareil.
