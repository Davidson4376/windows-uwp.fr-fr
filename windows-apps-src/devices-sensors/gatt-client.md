...
Cet article présente l’utilisation des API pour applications de plateforme Windows universelle (UWP), ainsi qu’un exemple de code pour les tâches courantes de client GATT Client attribut générique (GATT Bluetooth):
...
```
En revanche, de toutes les références à un BluetoothLEDevice supprimer l’objet d’un appareil (et si aucune autre application sur le système ne dispose d’une référence à l’appareil) déclenche une automatique déconnecter après un délai d’expiration petite. 

```csharp
bluetoothLeDevice.Dispose();
```
Si l’application a besoin d’accéder à l’appareil à nouveau, il vous suffit recréer l’objet de périphérique et accéder à une caractéristique (abordée dans la section suivante) déclenche le système d’exploitation pour vous reconnecter lorsque cela est nécessaire. Si l’appareil se trouve à proximité, vous obtiendrez l’accès à l’appareil dans le cas contraire, qu'elle renvoie avec une erreur DeviceUnreachable.  
...
Assurez-vous que la caractéristique prend en charge indiquer ou avertir (Vérifiez les propriétés caractéristiques pour vous assurer que). 

> **Mettre de côté**: indiquer est considéré comme plus fiable dans la mesure où chaque événement valeur modifiée est couplée avec un accusé de réception à partir de l’appareil du client. Avertir n’est plus répandus, car la plupart des transactions GATT seraient plutôt économiser l’énergie au lieu d’être extrêmement fiable. Dans tous les cas, tout cela est gérée sur la couche du contrôleur afin que l’application ne pas obtenir impliquée. Nous désignons collectivement leur sous forme de simplement «notifications», mais vous savez désormais. 

Il existe deux éléments à prendre en charge avant d’obtenir des notifications:
- Écrire dans le descripteur de Configuration caractéristique de Client (CCCD)
...
```
À présent, l’événement ValueChanged de la GattCharacteristic est appelé chaque fois que la valeur obtient modifiée sur l’appareil distant. Il reste consiste à implémenter le Gestionnaire d’événements: 
