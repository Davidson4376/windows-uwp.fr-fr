---
author: muhsinking
title: Prise en main de la technologie de point de vente
...
Les appareils de point de vente (PDV), aussi appelés appareils de point de service (POS), sont des périphériques informatiques utilisés pour faciliter les transactions de vente au détail. Les appareils de PDV incluent notamment les caisses enregistreuses électroniques, les scanneurs de code-barres, les lecteurs de bandes magnétiques et les imprimantes de reçus.

Vous allez apprendre ici les principes fondamentaux d’interfaçage avec des appareils de PDV à l’aide des API de PDV de la plateforme Windows universelle (UWP). Nous aborderons l’énumération d'appareils, la vérification des fonctionnalités des appareils, la revendication d'appareils et le partage d'appareils. Dans cet exemple, nous utilisons un scanneur de code-barres, mais presque tous les conseils fournis ici s’appliquent à l'ensemble des appareils de PDV compatibles UWP. (Pour obtenir la liste des appareils pris en charge, voir [Prise en charge des appareils de PDV](pos-device-support.md)).

## <a name="finding-and-connecting-to-pos-peripherals"></a>Recherche et connexion aux appareils de PDV

Avant qu'un appareil de PDV puisse être utilisé par une application, il doit être couplé au PC sur lequel l’application s’exécute. Il existe plusieurs façons de se connecter aux appareils de PDV, soit par programmation, soit par le biais de l’application Paramètres.

### <a name="connecting-to-devices-by-using-the-settings-app"></a>Connexion aux appareils à l’aide de l’application Paramètres
Lorsque vous connectez un appareil de PDV comme un scanneur de code-barres à un PC, il apparaît comme n'importe quel autre appareil. Vous pouvez le trouver dans la section **Appareils > Bluetooth et autres paramètres d’appareils** de l’application Paramètres. Pour effectuer un couplage avec un appareil de PDV, sélectionnez **Ajouter un appareil Bluetooth ou un autre appareil**.

Certains appareils de PDV peuvent ne pas apparaître dans l’application Paramètres tant qu'ils n'ont pas été énumérés par programmation à l’aide des API de PDV.

### <a name="getting-a-single-pos-device-with-getdefaultasync"></a>Obtention d’un seul appareil de PDV avec GetDefaultAsync
Dans un cas d’utilisation simple, vous pouvez n'avoir qu'un seul appareil de PDV connecté au PC sur lequel l’application s'exécute, et souhaiter configurer celui-ci aussi rapidement que possible. Pour ce faire, récupérez l’appareil «par défaut» avec la méthode **GetDefaultAsync**, comme illustré ici.
...
> [!NOTE] 
> Si plusieurs appareils de PDV sont connectés au PC, **GetDefaultAsync** renvoie le premier appareil détecté. Pour cette raison, utilisez **FindAllAsync**, sauf si êtes sûr qu’un seul appareil de PDV est visible par l’application.
...
La classe **Device.Capabilities** est un attribut de toutes les classes d'appareils de PDV, qui peut être utilisé pour obtenir des informations générales sur chaque appareil. Ainsi, cet exemple détermine si un appareil prend en charge la création de rapports statistiques et, si tel est le cas, récupère des statistiques pour tous les types pris en charge.
...
## <a name="claiming-a-pos-device"></a>Revendication d'un appareil de PDV
Avant de pouvoir utiliser un appareil de PDV pour des entrées ou sorties actives, vous devez le revendiquer en accordant à l'application l’accès exclusif à la plupart de ses fonctions. Ce code indique comment revendiquer un scanneur de code-barres, une fois que vous avez trouvé l’appareil en utilisant l’une des méthodes décrites précédemment.
...
Lorsque vous utilisez un appareil de PDV via une connexion réseau ou Bluetooth, il se peut que vous soyez amené à partager l’appareil avec d’autres applications sur le réseau. (Pour plus d’informations à ce sujet, voir Partage d'appareils[link].) Dans d'autres cas, vous voudrez peut-être rester connecté à l’appareil pour une utilisation prolongée. Cet exemple montre comment conserver un scanneur de code-barres revendiqué après qu’une autre application a demandé la libération de l’appareil.
...
> [!NOTE]
> Les classes d'appareils de PDV revendiquées et non revendiquées implémentent l'[interface IClosable](https://docs.microsoft.com/en-us/uwp/api/windows.foundation.iclosable). Si un appareil est connecté à une application via le réseau ou Bluetooth, les objets revendiqués et non revendiqués doivent être supprimés avant qu'une autre application puisse s'y connecter.
...
+ [Exemple d’imprimante de PDV](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/PosPrinter)
