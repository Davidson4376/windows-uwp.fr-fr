...
<br/>
Afficher un sélecteur d’appareil de l’interface utilisateur et demander à l’utilisateur de choisir un appareil connecté. Cette méthode gère la mise à jour de la liste lorsque les périphériques sont reliés et supprimés et est plus simple et plus sûre que d’autres méthodes.

**Méthode 2:** [Obtenir le premier périphérique disponible](#Method-1:-get-first-available-device)<br />[GetDefaultAsync](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodescanner.getdefaultasync) permet d’accéder au premier périphérique disponible dans une classe de périphérique de Point de Service spécifique.
...
Le moyen le plus simple d’obtenir un appareil de Point de Service consiste à utiliser des **GetDefaultAsync** pour obtenir le premier périphérique disponible au sein d’une classe de périphérique de Point de Service. 
...
> [!CAUTION]
> **GetDefaultAsync** doit être utilisé avec précaution car elle peut retourner un autre appareil à partir d’une session à l’autre. De nombreux événements peuvent influencer cette énumération pour donner un résultat de premier périphérique disponible différent, notamment: 
...
* [Classe de DeviceInformation](https://docs.microsoft.com/uwp/api/windows.devices.enumeration.deviceinformation)
