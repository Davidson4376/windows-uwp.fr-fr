...
Les objets [**XamlLight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.xamllight) permettent d’appliquer des classes [**CompositionLight**](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionLight) pour éclairer les éléments UIElements XAML de manière dynamique. XamlLight fournit des méthodes pour le ciblage d’éléments d’interface utilisateur ou les formes XAML, l’application de lumières à arborescences des éléments d’interface utilisateur, et aider à gérer la durée de vie de CompositionLight ressources selon qu’ils soient actuellement.
...
Cet exemple montre la définition d’un XamlLight personnalisé auquel s’applique une lumière multicolore ciblé des éléments d’interface utilisateur et des formes.
...
Vous pouvez ensuite appliquer ce clair à n’importe quel XAML UIElement ou un pinceau pour les clair. Cet exemple illustre différentes utilisations potentielles.

> [!Important]
> Pour [C + / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), supprimez les deux occurrences de `local:OrangeSpotLight.IsTarget="True"` à partir du balisage ci-dessous. Les propriétés attachées sont déjà définies dans le fichier code-behind.
...
![Exemples d’éléments éclairé par une lumière xaml](images/orange-spot-light.png)
