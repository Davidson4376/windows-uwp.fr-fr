...
- Transformation d’une lumière, indépendamment des autres objets de la scène pour activer les scénarios immersives comme les scènes de la lecture de musique.
...
[CompositionLight](https://docs.microsoft.com/uwp/api/windows.ui.composition.compositionlight) vous permet de créer différentes lumières et placez-les dans l’espace de coordonnées. Ces lumières ciblent des éléments visuels que vous souhaitez identifier comme éclairés.

### <a name="light-types"></a>Types de lumière

| Type | Description |
| --- | --- |
| [AmbientLight](/uwp/api/windows.ui.composition.ambientlight) | Une source de lumière émettant de la lumière non directionnelle qui apparaît réfléchie par tous les éléments de la scène. |
...
En ajoutant une animation pour le décalage de la lumière à points, un effet scintillent s’effectue facilement.
...
**Cibles et espace de coordonnées** | CoordinateSpace est alors utilisée est l’espace visuel dans lequel toutes les propriétés de lumières doivent être définies. CompositionLight.Targets doit être comprise dans l’arborescence CoordinateSpace est alors utilisée.
...
**Direction** | La direction de la lumière. La direction dans laquelle pointe la lumière est spécifiée par rapport à sa Visual [CoordinateSpace est alors utilisée](/uwp/api/windows.ui.composition.distantlight.coordinatespace) .
