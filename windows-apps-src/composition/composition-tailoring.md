...
Les utilisateurs peuvent personnaliser leur expérience de Windows pour diverses raisons, dont les applications doivent respecter les et s’adapte à. Une zone, les utilisateurs finaux peuvent contrôler les types d’effets qu'ils voient s’afficher utilisés tout au long de leur système change.

### <a name="transparency-effects-settings"></a>Paramètres des effets de transparence

Une tels paramètre d’effet les utilisateurs peuvent personnaliser est activer/désactiver les effets de transparence. Cela se trouvent dans l’application paramètres sous Personnalisation > couleurs, ou via l’application Paramètres > d’ergonomie > affichage.
...
Quand il sont désactivés, matière ACRYLIQUE automatiquement reviennent à une couleur unie, car cet événement pinceau ACRYLIQUE du code XAML a écouté par défaut. Ici, nous voir l’application Calculatrice correctement revenir à une couleur unie lorsque les effets de transparence ne sont pas activées:
...
Cette API fournit des méthodes et un écouteur d’événements qui peut être utilisé pour rendre l’effet de mise à l’échelle de décisions pour l’interface utilisateur de l’application. La fonctionnalité détecte la façon dont le système peut gérer composition complexe et opérations de rendu, puis retourne les informations dans un modèle facile à utiliser pour les développeurs à utiliser.
...
1. Ajouter du contenu à la méthode de rappel d’événement pour gérer les différents niveaux de fonctionnalités. Cela peut ou peut ne pas être similaire à l’étape suivante.
