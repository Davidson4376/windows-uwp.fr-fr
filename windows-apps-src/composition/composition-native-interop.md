...
L’exemple de code suivant illustre un scénario d’interopérabilité. L’exemple combine des types à partir de la zone de surface Windows Runtime de Composition de Windows, ainsi que les types à partir de l’interopérabilité en-têtes et le code qui restitue le texte à l’aide de DirectWrite basé sur COM et l’API de Direct2D. L’exemple utilise [ **BeginDraw** ](https://msdn.microsoft.com/library/windows/apps/mt620059.aspx) et [ **EndDraw** ](https://msdn.microsoft.com/library/windows/apps/mt620060) pour le rendre transparente pour interagir entre ces technologies. L’exemple utilise DirectWrite pour disposer le texte, et puis elle utilise Direct2D pour l’afficher. Le périphérique graphique de composition accepte directement le périphérique Direct2D au moment de l’initialisation. Cela permet de **BeginDraw** pour retourner un **ID2D1DeviceContext** pointeur d’interface, qui est beaucoup plus efficace que l’application créer un contexte de Direct2D pour encapsuler un retourné Interface ID3D11Texture2D à chaque opération de dessin.
