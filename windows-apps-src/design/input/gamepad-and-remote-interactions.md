...
***De nombreuses expériences d’interaction sont partagés entre le boîtier de commande, télécommande et le clavier***

Créer des expériences d’interaction dans vos applications de plateforme Windows universelle (UWP) qui vous assurer que votre application est accessible par le biais de ces deux les types d’entrée traditionnels des PC, ordinateurs portables et tablettes (souris, clavier, tactile, etc.), ainsi que les types d’entrées et utilisable par défaut de l’expérience *«10-foot»* Xbox et télévision, par exemple, le boîtier de commande et la télécommande.

Pour obtenir des conseils généraux de conception sur les applications UWP dans l’expérience *«10-foot»* , consultez [la conception pour Xbox et télévision](../devices/designing-for-tv.md) .

## <a name="overview"></a>Vue d'ensemble

Dans cette rubrique, nous abordons ce que vous devez prendre en compte dans votre conception de l’interaction (ou ce que vous n’avez pas, si la plateforme s’affiche après celle-ci pour vous) et fournir des conseils, des conseils et suggestions pour la création d’applications UWP qui sont plus agréables à utiliser indépendamment du périphérique, type d’entrée, ou capacités utilisateur et préférences.

Récapitulatif, votre application doit être aussi intuitives et faciles à utiliser dans l’environnement de *«2-foot»* qu’elle est dans l’environnement de *«10-foot»* (et vice versa). Prend en charge les périphériques par défaut de l’utilisateur, l’interface utilisateur stratégique clairement, organisent le contenu afin que la navigation soit prévisible et cohérente et donnent aux utilisateurs le chemin d’accès le plus court possible à ce qu’il souhaite effectuer.
...
## <a name="optimize-for-both-2-foot-and-10-foot-experiences"></a>Optimiser les expériences «10-foot» et «2-foot»

Au minimum, nous vous recommandons de tester vos applications pour vous assurer qu’ils fonctionnent bien dans les scénarios à la fois «2-foot» et «10-foot», et que toutes les fonctionnalités sont détectable et accessible par le Xbox le [boîtier de commande et de contrôle à distance](#gamepad-and-remote-control).

Voici d’autres façons, vous pouvez optimiser votre application pour une utilisation dans des expériences «10-foot» et «2-foot» et des périphériques d’entrée tout (chaque est liée à la section appropriée dans cette rubrique).

> [!NOTE]
> Dans la mesure où les télécommandes et boîtiers de commande Xbox prennent en charge de nombreux comportements de clavier UWP et expériences, ces recommandations sont appropriées pour les deux types d’entrée. Pour plus d’informations au clavier, voir [interactions avec le clavier](keyboard-interactions.md) .

| Fonctionnalité        | Description           |
| -------------------------------------------------------------- |--------------------------------|
| [Interaction et navigation en mode focusXY](#xy-focus-navigation-and-interaction) | **Navigation en mode focus XY** permet à l’utilisateur de naviguer dans l’interface utilisateur de votre application. Toutefois, cela limite la navigation à quatre directions: haut, bas, gauche et droite. Cette section apporte des recommandations pour y remédier ainsi que d’autres considérations. |
| [Mode souris](#mouse-mode)|Navigation en mode focus XY n’est pas pratique, ou même possible, pour certains types d’applications, telles que les cartes ou de dessin et de peinture des applications. Dans ces cas, **le mode souris** permet aux utilisateurs de naviguer librement avec un boîtier de commande ou un contrôle à distance, tout comme la souris sur un PC.|
| [Visuel du focus](#focus-visual)  | Le visuel du focus est une bordure qui met en évidence l’élément d’interface utilisateur ayant actuellement le focus. Cela permet à l’utilisateur d’identifier rapidement l’interface utilisateur qu’ils sont Parcourir ou d’interagir avec.  |
| [Activation du focus](#focus-engagement) | Activation du focus nécessite que l’utilisateur appuie sur le bouton **A/sélectionner** sur un boîtier de commande ou un contrôle à distance quand un élément d’interface utilisateur a le focus afin d’interagir avec celui-ci. |
| [Boutons matériels](#hardware-buttons) | Le boîtier de commande et la télécommande fournissent des configurations et des boutons très différents. |
...
\*Lorsque les événements [KeyDown](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keydown) et [KeyUp](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keyup) pour le boutonB ne sont pas gérés par l’application, l’événement [SystemNavigationManager.BackRequested](https://docs.microsoft.com/uwp/api/windows.ui.core.systemnavigationmanager.backrequested) est déclenché, ce qui doit se traduire par une navigation vers l’arrière au sein de l’application. Cependant, vous devez implémenter cela vous-même, comme dans l’extrait de code suivant:
...
Vous pouvez créer des applications UWP qui sont optimisées pour un appareil spécifique ou une expérience, mais la plateforme Windows universelle vous permet de créer des applications qui peuvent être utilisées avec succès sur tous les appareils, dans les expériences «10-foot» et «2-foot» et quels que soient l’entrée possibilité d’appareil ou l’utilisateur. En utilisant les recommandations de cet article permettre vous assurer que votre application fonctionne aussi bien que possible sur le téléviseur et un PC.
