...
- utiliser des constructions de XAML comme les [animations de Storyboarded](storyboarded-animations.md), ou le _* ThemeTransition_ et _* ThemeAnimation_ classes dans l’espace de noms [Windows.UI.Xaml.Media.Animation](/uwp/api/windows.ui.xaml.media.animation) .
...
À l’aide de la couche visuelle offre de meilleures performances que l’utilisation du code XAML crée. Toutefois, à l’aide de [ElementCompositionPreview](/uwp/api/Windows.UI.Xaml.Hosting.ElementCompositionPreview) pour obtenir [visuel](/uwp/api/windows.ui.composition.visual) objet de composition sous-jacent l’élément et puis animer l’élément visuel avec des animations de composition, sont plus complexe à utiliser.
...
## <a name="new-rendering-properties-replace-old-rendering-properties"></a>Nouvelles propriétés de rendu remplacement les propriétés de rendu ancien
...
Ces propriétés ont le même objectif et le même comportement que les propriétés de même nom sur la composition [visuelle](/uwp/api/windows.ui.composition.visual) classe (à l’exception de traduction, qui n’est pas sur l’élément visuel).
...
Lorsque vous définissez (ou animez) une des nouvelles propriétés, vous ne pouvez pas utiliser les propriétés anciennes. À l’inverse, si vous définissez (ou animez) les propriétés anciennes, vous ne pouvez pas utiliser les nouvelles propriétés.
...
Utiliser les méthodes [StartAnimation](/uwp/api/windows.ui.xaml.uielement.startanimation) et [StopAnimation](/uwp/api/windows.ui.xaml.uielement.stopanimation) sur UIElement d’animer les propriétés de l’élément UIElement.
...
Une page possède deux boutons. Le deuxième bouton s’anime pour être deux fois plus large (par l’intermédiaire de l’échelle) en tant que le premier bouton.
