---
author: mijacobs
description: Type de pinceau qui crée une texture transparente.
...
:::row:::
    :::column:::
        ACRYLIQUE de thème clair ![ACRYLIQUE de thème clair](images/Acrylic_LightTheme_Base.png)
    :::column-end:::
    :::column:::
        ACRYLIQUE de thème foncé ![ACRYLIQUE de thème foncé](images/Acrylic_DarkTheme_Base.png)
    :::column-end:::
:::row-end:::
...
:::row:::
    ::: étendue de colonnes::: ![certains d’image](images/XAML-controls-gallery-app-icon.png)
    :::column-end:::
    ::: colonne étendue = «2»::: **Galerie de contrôles XAML**<br>
        Si vous avez l’application de la galerie de contrôles XAML installée, cliquez <a href="xamlcontrolsgallery:/item/Acrylic">ici</a> pour ouvrir l’application et de voir ACRYLIQUE à le œuvre.
...
Si votre application n’est pas en mesure de tirer parti de NavigationView et que vous prévoyez d’ajouter ACRYLIQUE sur votre propre, nous recommandons d’utiliser ACRYLIQUE relativement transparent avec opacité de la teinte de 60 %.
...
> [!Note]
> Rendu surfaces ACRYLIQUES consomme processeur, ce qui peut augmenter la consommation d’énergie périphérique et réduire la durée de vie. Effets ACRYLIQUES sont désactivées automatiquement lorsque des périphériques en mode veille batterie, et les utilisateurs peuvent désactiver les effets ACRYLIQUES pour toutes les applications, s’ils le souhaitent.


## <a name="acrylic-blend-types"></a>Types de lissage acrylique
Caractéristique majeures d’ACRYLIQUE est sa transparence. Il existe deux types de lissage acrylique qui modifient ce qui est visible via le support:
...
En mode Contraste élevé, les utilisateurs continuent de voir la couleur d’arrière-plan qu'ils ont choisie, au lieu de l'acrylique. En outre, ACRYLIQUE d’arrière-plan et d’ACRYLIQUE dans l’application apparaissent sous la forme d’une couleur unie:
 - Lorsque l’utilisateur désactive la transparence dans Paramètres > Personnalisation > couleur
 - Lorsque la batterie économiseur d’écran est activée
 - Lorsque l’application est exécutée sur un matériel bas de gamme

En outre, uniquement les ACRYLIQUE arrière-plan remplacera la transparence et la texture d’une couleur unie:
...
<table>
    ...
    <tr>
        <td> <b>Recommandé d’utilisation:</b> Si votre application utilise le texte secondaire de couleur AltMedium avec une taille de texte de 18px ou plus, vous pouvez placer ces ressources ACRYLIQUE 70 % plus transparents derrière le texte. Nous vous recommandons d’utiliser ces ressources dans les éléments de navigation horizontale supérieure et dans les zones des éléments de contrôle de votre application.  </td>
    </tr>
    <tr>
...
Vous pouvez choisir d'ajouter une teinte à l'acrylique de votre application pour afficher une marque ou assurer un équilibre visuel avec les autres éléments de la page. Pour afficher la couleur plutôt que des nuances de gris, vous devez définir vos propres pinceaux acryliques en utilisant les propriétés suivantes.
 ...
 - **TintOpacity**: l’opacité de la couche de teinte. Nous vous recommandons 80 % d’opacité comme point de départ, bien que des couleurs différentes peut se présenter plus attrayant à d’autres translucencies.
