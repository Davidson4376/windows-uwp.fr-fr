---
author: serenaz
description: Z en profondeur, ou relatif profondeur et les clichés instantanés sont deux façons de profondeur d’intégrer dans votre application pour aider les utilisateurs à se concentrer naturellement et plus efficacement.
...
![profondeur de z](images/elevation-shadow/elevation.svg)

### <a name="why-use-z-depth"></a>Pourquoi utiliser z en profondeur?

Dans le monde physique, nous ont tendance à se concentrer sur les objets qui sont plus proches de nous. Nous pouvons appliquer ce instinct spatial à l’interface utilisateur numérique, également. Par exemple, si vous concevez un élément le plus proche pour l’utilisateur, puis l’utilisateur sera instinctivement centrée sur l’élément. En déplacement éléments d’interface utilisateur plus près de l’axe z, vous pouvez établir une hiérarchie visuelle entre les objets, aider les utilisateurs à effectuer des tâches naturellement et efficacement dans votre application. 

![z en profondeur dans le menu de contenu](images/elevation-shadow/whyelevation.svg)

En plus de fournir une hiérarchie visuelle significative, z en profondeur permet également de vous permet de créer expériences fluides en toute transparence de la 2D à des environnements 3D, mise à l’échelle de votre application sur tous les appareils et les facteurs de forme. 
...
En fonction de la façon dont nous percevoir profondeur dans le monde physique, Voici plusieurs techniques qui peuvent être utilisés pour afficher la proximité dans l’interface utilisateur numérique.
...
- **Mouvement** Vitesse relative peut être utilisée pour illustrer la proximité: déplacent des objets plus proches plus rapidement que les objets distants en arrière-plan. Pour savoir comment implémenter cet effet, voir [l’effet parallaxe](../motion/parallax.md).
...
![profondeur z au sein d’une application](images/elevation-shadow/app-depth.svg)

## <a name="what-is-shadow"></a>Quel est le shadow?

![shadow](images/elevation-shadow/shadow.svg)

Ombre est un moyen de percevoir une élévation. En cas de lumière au-dessus d’un objet avec élévation de privilèges, il existe une ombre sur la surface ci-dessous. Plus l’objet, la plus grande et plus douce l’ombre devient. Notez que les objets avec élévation de privilèges ne doivent avoir des ombres, mais les ombres indiquent une élévation.
...
Le ThemeShadow type peut être appliqué à n’importe quel élément XAML pour dessiner des ombres de façon appropriée en fonction de x, y, z coordonnées. ThemeShadow ajuste automatiquement pour les autres conditions ambiantes:
...
- Maintient les éléments synchronisées dans leur déplacement et de modifier une élévation.
...
- [Suggestion automatique](../controls-and-patterns/auto-suggest-box.md), [zone de liste déroulante](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ComboBox), les [sélecteurs de calendrier et Date/heure](../controls-and-patterns/date-and-time.md), [info-bulle](../controls-and-patterns/tooltips.md)
