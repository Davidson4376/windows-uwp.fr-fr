...
ms.locfileid: "3374501"
---
# <a name="buttons"></a>Boutons

> [!IMPORTANT]
> Cet article décrit des fonctionnalités qui n’ont pas encore été publiées et sont susceptibles d’être considérablement modifiées d’ici leur commercialisation. Microsoft ne donne aucune garantie, expresse ou implicite, concernant les informations fournies ici. Fonctionnalités d’aperçu nécessitent [dernière génération de l’aperçu de Windows 10 opérations d’initiés et Kit de développement](https://insider.windows.com/for-developers/) ou la [Bibliothèque de l’interface utilisateur de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un bouton permet à l’utilisateur de déclencher une action immédiate. Certains boutons sont spécialisées pour des tâches spécifiques, telles que la navigation, les actions répétées ou présenter des menus.

![Exemple de boutons](images/controls/button.png)

Le framework XAML fournit un contrôle de bouton standard ainsi que plusieurs contrôles spécialisés.
...
[RepeatButton](/uwp/api/windows.ui.xaml.controls.primitives.repeatbutton) | Déclenche un événement Click en permanence lorsque l’utilisateur appuie sur un bouton.
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) | (Aperçu) Un bouton avec un chevron pour ouvrir une fenêtre mobile attaché.
[SplitButton](/uwp/api/windows.ui.xaml.controls.splitbutton) | (Aperçu) Un bouton à deux côtés. Un côté initie une action et un menu s’affiche de l’autre côté.
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Aperçu) Un bouton bascule avec deux côtés. Un côté bascule activé/désactivé, et l’autre côté affiche un menu.

| **Obtenir la bibliothèque d’interface utilisateur Windows** |
| - |
| DropDownButton SplitButton et ToggleSplitButton sont inclus dans le cadre de la bibliothèque de l’interface utilisateur de Windows, un package NuGet qui contient des contrôles et des fonctionnalités d’interface utilisateur pour les applications de la série UWP. Pour plus d’informations, y compris les instructions d’installation, consultez la [vue d’ensemble de la bibliothèque de l’interface utilisateur de Windows](https://docs.microsoft.com/uwp/toolkits/winui/). |

| **API de la plate-forme** | **API de bibliothèque de l’interface utilisateur Windows** |
| - | - |
| [Cliquez sur un événement](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click), [propriété de commande](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.command) | La [classe DropDownButton](/uwp/api/microsoft.ui.xaml.controls.dropdownbutton), [classe de SplitButton](/uwp/api/microsoft.ui.xaml.controls.splitbutton), [classe de ToggleSplitButton](/uwp/api/microsoft.ui.xaml.controls.togglesplitbutton) |

## <a name="is-this-the-right-control"></a>Est-ce le contrôle approprié?

Utiliser un **bouton** pour permettre à l’utilisateur de lancer une action immédiate, par exemple l’envoi d’un formulaire.

N’utilisez pas un bouton lorsque l’action consiste à naviguer vers une autre page; Utilisez un [contrôle HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) . Voir [Liens hypertexte](hyperlinks.md) pour plus d’informations.
> Exception: pour la navigation dans un assistant, utilisez les boutons Précédent et Suivant. Pour les autres types de descendante de navigation ou de navigation à un niveau supérieur, utilisez un [bouton précédent](../basics/navigation-history-and-backwards-navigation.md).

Utilisez un **RepeatButton** lorsque l’utilisateur souhaite déclencher une action à plusieurs reprises. Par exemple, utiliser un RepeatButton pour incrémenter ou décrémenter une valeur dans un compteur.

Utilisez un **DropDownButton** lorsque le bouton a un lanceur qui contient plus d’options. Le chevron par défaut fournit une indication visuelle que le bouton inclut un lanceur.

Utiliser un **bouton partagé** lorsque vous souhaitez que l’utilisateur soit en mesure de lancer une action immédiate ou choisissez indépendamment des options supplémentaires.
...
> **Aperçu**: DropDownButton nécessite [dernière génération de l’aperçu de Windows 10 opérations d’initiés et Kit de développement](https://insider.windows.com/for-developers/) ou la [Bibliothèque de l’interface utilisateur de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un [DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) est un bouton qui représente un chevron sous la forme d’un indicateur visuel qu’il a un lanceur joint qui contient plus d’options. Il a le même comportement qu’un bouton standard avec un lanceur; seule l’apparence diffère.

Le bouton déroulant hérite de l’événement Click, mais vous ne l’utilisez généralement. Au lieu de cela, vous utilisez la propriété Lanceur joindre un lanceur et d’appeler des actions à l’aide des options de menu dans la fenêtre mobile. La fenêtre mobile s’ouvre automatiquement lorsque l’utilisateur clique sur le bouton.

> [!TIP]
> Pour plus d’informations sur les lanceurs, consultez [Menus et menus contextuels](menus.md).

### <a name="example---drop-down-button"></a>Exemple - bouton déroulant

Cet exemple montre comment créer un bouton déroulant avec un lanceur qui contient des commandes pour l’alignement des paragraphes dans un RichEditBox. (Pour plus d’informations et de code, consultez [Rich edit zone](rich-edit-box.md)).
...
> **Aperçu**: SplitButton nécessite [dernière génération de l’aperçu de Windows 10 opérations d’initiés et Kit de développement](https://insider.windows.com/for-developers/) ou la [Bibliothèque de l’interface utilisateur de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un [bouton partagé](/uwp/api/windows.ui.xaml.controls.splitbutton) a deux parties qui peuvent être appelées séparément. Une partie se comporte comme un bouton standard et appelle une action immédiate. L’autre partie appelle un lanceur qui contient des options supplémentaires que l’utilisateur peut choisir.

> [!NOTE]
> Lorsqu’elle est appelée avec un contact, le bouton partagé se comporte comme un bouton déroulant; les deux moitiés du bouton appellent la fenêtre mobile. Avec d’autres méthodes d’entrée, un utilisateur peut appeler soit moitié du bouton séparément.
...
- Lorsque la liste déroulante est ouverte, appel handle des articles dans la liste déroulante pour les deux modifications dont l’option est sélectionnée, puis appelez-le. Il est important d’appeler l’élément mobile, car l’événement Click ne se produit lors de l’utilisation tactile de bouton.

> [!TIP]
> Il existe de nombreuses façons de placer des éléments dans la liste déroulante vers le bas et gérer leur appel. Si vous utilisez un ListView ou le GridView, une consiste à gérer l’événement SelectionChanged. Si vous le faites, la valeur [SingleSelectionFollowsFocus](/uwp/api/windows.ui.xaml.controls.listviewbase.singleselectionfollowsfocus) sur **false**. Cela permet aux utilisateurs de parcourir les options à l’aide d’un clavier sans appeler l’article de chaque modification.

### <a name="example---split-button"></a>Exemple - bouton de fractionnement

Cet exemple montre comment créer un bouton partagé qui est utilisé pour modifier la couleur de premier plan du texte sélectionné dans un RichEditBox. (Pour plus d’informations et de code, consultez [Rich edit zone](rich-edit-box.md)).
...
## <a name="create-a-toggle-split-button"></a>Créer un bouton de fractionnement

> **Aperçu**: ToggleSplitButton requiert [dernière génération de l’aperçu de Windows 10 opérations d’initiés et Kit de développement](https://insider.windows.com/for-developers/) ou la [Bibliothèque de l’interface utilisateur de Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

Un [ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) a deux parties qui peuvent être appelées séparément. Une partie se comporte comme un bouton bascule qui peut être activé ou désactivé. L’autre partie appelle un lanceur qui contient des options supplémentaires que l’utilisateur peut choisir.

Un bouton de fractionnement est utilisé en général pour activer ou désactiver une fonctionnalité lorsque la fonctionnalité possède plusieurs options que l’utilisateur peut choisir. Par exemple, dans un éditeur de document, il peut servir à activer ou désactiver, les listes tandis que la liste déroulante permet de choisir le style de la liste.

> [!NOTE]
> Lorsqu’elle est appelée avec un contact, le bouton partagé se comporte comme un bouton déroulant. Avec d’autres méthodes d’entrée, un utilisateur peut appeler soit moitié du bouton séparément. Tactile, les deux moitiés du bouton appellent la fenêtre mobile. Par conséquent, vous devez inclure une option dans votre contenu mobile pour activer / désactiver le bouton.

### <a name="differences-with-togglebutton"></a>Différences avec le contrôle ToggleButton

Contrairement au [contrôle ToggleButton](/uwp/api/windows.ui.xaml.controls.primitives.togglebutton), ToggleSplitButton n’a pas un état indéterminé. Par conséquent, vous gardez à l’esprit ces différences:

- ToggleSplitButton n’a pas un propriété **IsThreeState est** ou **indéterminé** .
- La propriété [ToggleSplitButton.IsChecked](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischecked) est simplement un **bool**, pas un **bool nullable**.
- ToggleSplitButton a seulement l’événement [IsCheckedChanged](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischeckedchanged) . Il n’a pas d’événements distincts de **Checked** et **Unchecked** .

### <a name="example---toggle-split-button"></a>Exemple - bascule de bouton partagé

L’exemple suivant montre comment un bouton partagé peut être utilisé pour activer ou désactiver la mise en forme de liste et modifier le style de la liste, dans un RichEditBox. (Pour plus d’informations et de code, consultez [Rich edit zone](rich-edit-box.md)).

![Un bouton de fractionnement de bascule pour la sélection des styles de liste](images/toggle-split-button-open.png)
