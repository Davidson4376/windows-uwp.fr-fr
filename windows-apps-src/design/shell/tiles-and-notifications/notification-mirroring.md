---
Description: Découvrez comment utiliser notification mise en miroir sur vos notifications de toast.
title: Mise en miroir des notifications
label: Notification mirroring
template: detail.hbs
ms.date: 12/15/2017
ms.topic: article
keywords: windows 10, uwp, toast, centre de notifications dans le cloud, mise en miroir des notifications, notification, sur plusieurs appareils
...
> [!IMPORTANT]
> **Requiert la mise à jour anniversaire**: Vous devez exécuter build 14393 ou une version ultérieure pour afficher la notification de mise en miroir de travail. Si vous souhaitez ne pas utiliser la mise en miroir des notifications, vous devez cibler le Kit de développement logiciel (SDK) 14393 pour accéder aux API de mise en miroir.
...
Les développeurs obtenir deux grands avantages à partir de la notification de mise en miroir : Les notifications de mise en miroir entraînent plus l’intérêt des utilisateurs avec votre service, et également les utilisateurs à découvrir votre application de bureau Microsoft Store ! Vos utilisateurs peuvent ne même pas savoir qu’une application UWP extraordinaire est disponible pour leur bureau Windows 10. Quand les utilisateurs reçoivent la notification de mise en miroir à partir de leur numéro de téléphone, les utilisateurs peuvent cliquer la notification toast à prendre pour le Microsoft Store, où ils peuvent installer votre application de bureau UWP.
...
**Problème connu**: Récupération de la propriété de mise en miroir via la `ToastNotificationHistory.GetHistory()` l’API retourne toujours la valeur par défaut (**autorisé**) au lieu de l’option que vous avez spécifié. Ne vous inquiétez pas, tout est normal : seule la valeur incorrecte est récupérée.
...
Certains développeurs peuvent choisir de désactiver complètement la mise en miroir des notifications. Bien que nous pensons que toutes les applications tireront à partir de la mise en miroir, nous facilitons se désabonner. Simplement appeler la méthode suivante à la fois, et êtes désinscrit votre application. Par exemple, vous pouvez placer cet appel dans le constructeur de votre application à l’intérieur de `App.xaml.cs`...
