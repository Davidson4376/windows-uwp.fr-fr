---
Description: Exécutez Desktop Converter App pour créer un package d'application de bureau Windows (Win32, WPF, Windows Forms).
...
ms.locfileid: "57652544"
---
# <a name="package-a-desktop-application-using-the-desktop-app-converter"></a>Empaqueter une application de bureau à l’aide de la de Desktop App Converter

[Obtenir le convertisseur d’application de bureau](https://aka.ms/converter)

Le Desktop App Converter (DAC) crée des packages pour les applications bureautiques à intégrer avec les dernières fonctionnalités de Windows, y compris la distribution et la maintenance via le Microsoft Store. Cela inclut les applications Win32 et celles que vous avez créées à l’aide de .NET 4.6.1.
...
> [!NOTE]
> Obtenez <a href="https://mva.microsoft.com/en-US/training-courses/developers-guide-to-the-desktop-bridge-17373?l=oZG0B1WhD_8406218965/">cette série</a> de courtes vidéos publiées par Microsoft Virtual Academy. Ces vidéos vous expliquent certaines méthodes courantes d’utilisation de Desktop App Converter.
...
:heavy_check_mark: Inscrire vos serveurs COM publics.

**Mise à jour anniversaire de Windows 10 ou version ultérieure**

:heavy_check_mark: Signer automatiquement votre package pour vous permettre de tester votre application.

:heavy_check_mark: Valider votre application par rapport à l’application empaquetée en matière de Microsoft Store.
...
Vous pouvez passer à la section suivante si votre application n’a pas un programme d’installation.
...
1. Identifiez le numéro de version de votre système d’exploitation.
   ...
   ![Version de Windows 10](images/desktop-to-uwp/win-10-version.png)
...
* [Empaqueter une application qui dispose d’un fichier de programme d’installation (.msi)](#installer-conversion)
* [Empaqueter une application qui a un fichier exécutable d’installation](#setup-conversion)
* [Empaqueter une application qui n’a pas un programme d’installation](#no-installer-conversion)
* [Empaqueter une application, la signer l’application et la préparer pour la soumission de Store](#optional-parameters)

<a id="installer-conversion" />

#### <a name="package-an-application-that-has-an-installer-msi-file"></a>Empaqueter une application qui dispose d’un fichier de programme d’installation (.msi)
...
#### <a name="package-an-application-that-has-a-setup-executable-file"></a>Empaqueter une application qui a un fichier exécutable d’installation
...
```
>[!IMPORTANT]
>Si les partenaires assigne une identité à votre package commence par un nombre, assurez-vous que vous transmettez également dans le <i>- AppId</i> paramètre et utiliser uniquement le suffixe de chaîne (après le point de séparation) en tant que la valeur de ce paramètre.

Le paramètre ``InstallerArguments`` est facultatif. Toutefois, étant donné que le Desktop App Converter a besoin de votre programme d’installation à s’exécuter en mode sans assistance, vous devrez l’utiliser si votre application a besoin des indicateurs en mode silencieux à exécuter en mode silencieux. L’indicateur ``/S`` est un indicateur de mode silencieux très courant, mais celui que vous utilisez peut être différent selon les technologies utilisées pour créer le fichier d’installation.
...
#### <a name="package-an-application-that-doesnt-have-an-installer"></a>Empaqueter une application qui n’a pas un programme d’installation

Dans cet exemple, utilisons le ``Installer`` paramètre pour pointer vers le dossier racine de vos fichiers d’application.
...
Cet exemple est similaire au premier, à ceci près qu’il montre comment vous pouvez signer votre application pour le test local et puis validez votre application par rapport à l’application empaquetée en matière de Microsoft Store.
...
Le ``Sign`` paramètre génère un certificat, puis connecte votre application avec lui. Vous devrez installer ce certificat généré pour exécuter votre application. Pour savoir comment procéder, consultez la section [Exécuter l’application empaquetée](#run-app) de ce guide.

Vous pouvez valider application en utilisant le ``Verify`` paramètre.
...
|-Setup [&lt;SwitchParameter&gt;] |Obligatoire |Exécute DesktopAppConverter en mode installation. Le mode d’installation prend en charge le développement d’une image de base fournie.|
|-BaseImage &lt;chaîne&gt; | Obligatoire |Chemin d’accès complet vers une image de base non développée. Ce paramètre est obligatoire si -Setup est spécifié.|
...
|-NoRestart [&lt;SwitchParameter&gt;] |Obligatoire |N’exécutez pas de commande de redémarrage lors de l’exécution du programme d’installation (le redémarrage est nécessaire pour activer la fonctionnalité de conteneur). |
...
|-Destination &lt;chaîne&gt; |Obligatoire |La destination souhaitée pour la sortie d’appx du convertisseur : DesktopAppConverter peut créer cet emplacement s’il n’existe pas déjà.|
|-Installer &lt;chaîne&gt; |Obligatoire |Le chemin d’accès du programme d’installation de votre application doit être en mesure de s’exécuter sans assistance/silencieusement. Conversion de non-programme d’installation, il s’agit du chemin d’accès au répertoire racine des fichiers de votre application. |
...
|-Version &lt;Version&gt; |Obligatoire |Le numéro de version de votre package d’application Windows universelle |
|<a id="manifest-params" /><strong>Paramètres de manifeste de package</strong>||
|-AppExecutable &lt;chaîne&gt; |Facultatif |Nom de l’exécutable principal de votre application (par exemple, « MyApp.exe »). Ce paramètre est requis avec les conversions sans programme d’installation. |
...
|-AppId &lt;chaîne&gt; |Facultatif |Spécifie une valeur à partir de laquelle est définie l’ID d’application dans le manifeste du package d’application Windows. Si elle n’est pas spécifiée, elle sera définie à la valeur transmise pour *PackageName*. Dans de nombreux cas, l'utilisation de *PackageName* convient. Toutefois, si partenaires assigne une identité à votre package commence par un nombre, assurez-vous que vous également transmettez le <i>- AppId</i> paramètre et utiliser uniquement le suffixe de chaîne (après le point de séparation) en tant que la valeur de ce paramètre. |
|-AppDisplayName &lt;chaîne&gt;  |Facultatif |Spécifie une valeur à partir de laquelle est défini le nom complet de l’application dans le manifeste du package d’application Windows. Si elle n’est pas spécifiée, elle sera définie à la valeur transmise pour *PackageName*. |
|-AppDescription &lt;chaîne&gt; |Facultatif |Spécifie une valeur à partir de laquelle est définie la description d’application dans le manifeste du package d’application Windows. Si elle n’est pas spécifiée, elle sera définie à la valeur transmise pour *PackageName*.|
|-PackageDisplayName &lt;chaîne&gt; |Facultatif |Spécifie une valeur à partir de laquelle est défini le nom complet du package dans le manifeste du package d’application Windows. Si elle n’est pas spécifiée, elle sera définie à la valeur transmise pour *PackageName*. |
|-PackagePublisherDisplayName &lt;chaîne&gt; |Facultatif |Spécifie une valeur à partir de laquelle est défini le nom complet de l’éditeur du package dans le manifeste du package d’application Windows. Si elle n’est pas spécifiée, elle sera définie à la valeur transmise pour *Publisher*. |
|<a id="cleanup-params" /><strong>Paramètres de nettoyage</strong>|||
|-Cleanup [&lt;Option&gt;] |Obligatoire |Exécute le nettoyage pour les artefacts DesktopAppConverter. Il existe trois options valides pour le mode de nettoyage. |
...
L’une d’elles consiste à ouvrir une invite de commandes PowerShell pour saisir la commande suivante : ```Add-AppxPackage –Register AppxManifest.xml```. C’est probablement le moyen le plus simple pour exécuter votre application, car vous n’êtes pas obligé de le signer.

Une autre méthode consiste à signer votre application avec un certificat. Si vous utilisez le ```sign``` paramètre, le Desktop App Converter génère un pour vous et puis signer votre application avec elle. Ce fichier est nommé **auto-generated.cer**, et vous pouvez le trouver dans le dossier racine de votre application empaquetée.
...
Vous allez probablement apporter des modifications à votre application empaquetée pour résoudre des bogues, ajouter des ressources visuelles ou améliorer votre application avec des expériences modernes tels que des vignettes dynamiques.

Une fois vos modifications accomplies, il n’est pas nécessaire d’effectuer une nouvelle conversion. Dans la plupart des cas, vous pouvez simplement reconditionner votre application à l’aide de l’outil MakeAppx et le fichier appxmanifest.xml la DAC génère pour votre application. See [Générer un package d’application Windows](desktop-to-uwp-manual-conversion.md#make-appx).
...
|Modifier et générer un nouveau package |Démonstration : Modifier et générer un nouveau package|
|---|---|
...
Les deux sections suivantes décrivent quelques corrections facultatives pour l’application empaquetée que vous pouvez envisager.
...
**Exécutez votre application / rechercher et résoudre les problèmes**
