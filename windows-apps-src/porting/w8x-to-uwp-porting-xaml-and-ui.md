...
**Remarque**  l’état visuel PointerOver est adapté styles/modèles personnalisés dans les applications Windows 10 et dans les applications Windows Runtime 8.x, mais non dans les applications du Windows Phone Store. Pour cette raison (et, en raison de clés de ressources système qui sont pris en charge pour les applications Windows 10), nous recommandons que vous réutiliser les styles/modèles personnalisés à partir d’applications Windows Runtime 8.x quand vous portez votre application pour Windows 10.
Si vous voulez être certain que vos styles/modèles personnalisés utilisent le dernier jeu d’états visuels et bénéficient des améliorations de performances apportées aux styles/modèles par défaut, puis modifiez une copie du nouveau modèle par défaut de Windows 10 et réappliquez votre personnalisation de. Un exemple d’amélioration des performances correspond à la suppression de tous les éléments **Border** qui délimitaient précédemment un élément **ContentPresenter** ou Panel et au rendu de la bordure par un élément enfant.
...
| [**Hub**](https://msdn.microsoft.com/library/windows/apps/dn251843)  | Dans une application 8.1 universelle, la propriété [**HubSection.IsHeaderInteractive**](https://msdn.microsoft.com/library/windows/apps/dn251917) rend interactifs l’en-tête de section et le glyphe de chevron rendu en regard de ce dernier. Dans une application Windows 10, il existe une affordance interactive «En savoir plus» à côté de l’en-tête, mais l’en-tête proprement dit n’est pas interactif. **IsHeaderInteractive** détermine toujours si l’interaction déclenche l’événement [**Hub.SectionHeaderClick**](https://msdn.microsoft.com/library/windows/apps/dn251953). |
...
| [**TextBox**](https://msdn.microsoft.com/library/windows/apps/br209683) | Dans une application de 8.x Windows Runtime, la vérification orthographique est désactivée par défaut pour une [**zone de texte**](https://msdn.microsoft.com/library/windows/apps/br209683). Dans une application Windows Phone Store et dans une application Windows 10, elle est activée par défaut. |
...
| Divers | Couleur d’accentuation s’applique à un applications Windows Phone Store et pour les applications Windows 10, mais pas vers des applications Windows Runtime 8.x.  |
...
Nous vous déconseillons que vous essayez de prendre en charge tous les facteurs d’échelle, mais la liste complète des facteurs d’échelle pour les applications Windows 10 est de 100 %, 125 %, 150 %, 200 %, 250 %, 300 % et 400 %. Si vous fournissez ces facteurs, le Windows Store sélectionne les ressources de taille appropriée pour chaque appareil, et seules ces ressources sont téléchargées. Le Windows Store sélectionne les ressources à télécharger en fonction de la résolution de l’appareil. Vous pouvez réutiliser les ressources de votre application de 8.x Windows Runtime à des facteurs d’échelle tels que 140 % et 220 %, mais votre application s’exécutera dans l’un des nouveaux facteurs d’échelle et par conséquent, une image bitmap mise à l’échelle sera inévitable. Testez votre application sur divers appareils pour voir si vous êtes satisfait des résultats dans votre cas.

Vous pouvez être réutilisez le balisage XAML d’une application Windows Runtime 8.x dans les valeurs de dimensions littérales sont exploitées dans le balisage (pour le dimensionnement de formes ou d’autres éléments, par exemple, pour typographie). Mais, dans certains cas, un plus grand facteur d’échelle est utilisé sur un appareil pour une application Windows 10 que pour une application universelle 8.1 (par exemple, 150 % est utilisé dans lequel 140 % auparavant, et 200 % est où était de 180 %). Par conséquent, si vous trouvez que ces valeurs littérales sont trop élevées sur Windows 10, essayez de les multiplier par 0,8. Pour plus d’informations, voir [Conception réactive 101 pour les applications UWP](https://msdn.microsoft.com/library/windows/apps/dn958435).
...
Voici les aspects de [**GridView**](https://msdn.microsoft.com/library/windows/apps/br242705) et [**ListView**](https://msdn.microsoft.com/library/windows/apps/br242878) qui ont modifier ou ne sont pas pris en charge dans Windows 10.
...
Les API dans l’espace de noms [**Windows.Media.PlayTo**](https://msdn.microsoft.com/library/windows/apps/br207025) sont déconseillées pour les applications de Windows 10 en faveur de l’API [**Windows.Media.Casting**](https://msdn.microsoft.com/library/windows/apps/dn972568) .
...
Pour les clés qui sont toujours prises en charge, les modifications apportées au langage de conception signifient que les propriétés définies par certains styles ont changé. Par exemple, `TitleTextBlockStyle` affecte **FontSize** 14,667 px dans une application de 8.x Windows Runtime et sur 18,14 px dans une application Windows Phone Store. Toutefois, le même style définit **FontSize** sur une quantité plus élevée 24 px dans une application Windows 10. Passez en revue vos conceptions et dispositions et utilisez les styles appropriés aux endroits adéquats. Pour plus d’informations, voir [Recommandations en matière de polices](https://msdn.microsoft.com/library/windows/apps/hh700394.aspx) et [Concevoir des applications UWP](http://dev.windows.com/design).
...
Dans une application de 8.x Windows Runtime, lorsque l’application est désactivée et réactivée, la vue avec zoom arrière est masquée (si elle était affichée) et la vue avec zoom avant est illustrée à la place. Dans une application Windows Phone Store et dans une application Windows 10, la vue avec zoom arrière reste affichée si elle était affichée.
