...
| PSFLauncher | Il s’agit de projet C++ vide. Ce projet est un endroit pour collecter les fichiers distribuable runtime de l’infrastructure de prise en charge de Package. Elle génère un fichier exécutable. Cet exécutable est la première chose qui s’exécute lorsque vous démarrez la solution. |
...
Si vous n’avez pas encore une solution pour votre application de bureau, créez une nouvelle **Solution vierge** dans Visual Studio.
...
Dans l' **Explorateur de solutions**, cliquer sur le projet de création de packages et sélectionnez **Modifier**, puis ajoutez cela vers le bas du fichier de projet:
...
Si vous souhaitez déboguer ou étendre un correctif runtime existant, ajoutez les fichiers de correctif runtime que vous avez obtenu en utilisant les instructions décrites dans la section [trouver un correctif de runtime](#find) de ce guide.
...
Ajoutez un fichier nommé ``config.json`` à votre projet de création de package, puis, copiez et collez le texte json suivant dans le fichier. Définissez la propriété **Action de Package** au **contenu**.
...
| corrections | config | (Facultatif) Contrôle le comporte de la correction DLL. Le format exact de cette valeur varie sur une base de correction-par-correction comme chaque correction puisse les interpréter ce «blob» qu’il le souhaite. |
...
#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>Protection contre les appels récurrents à des fonctions de correctifs de runtime

Vous pouvez éventuellement appliquer le `reentrancy_guard` type à vos fonctions qui protègent contre les appels récurrents à des fonctions de correctifs de runtime.
