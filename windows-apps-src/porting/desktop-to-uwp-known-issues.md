...
ms.locfileid: "57637094"
---
# <a name="known-issues-with-packaged-desktop-applications"></a>Problèmes connus avec les applications de bureau standard

Cet article contient des problèmes connus qui peuvent se produire lorsque vous créez un package d’application Windows pour votre application de bureau.

<a id="app-converter" />

## <a name="known-issues-with-the-desktop-app-converter"></a>Problèmes connus avec Desktop App Converter
...
### <a name="new-containernetwork-the-object-already-exists-error"></a>New-ContainerNetwork : L’objet existe déjà erreur
...
Pour résoudre ce problème, essayez d’utiliser un programme d’installation spécifique à votre architecture (32 bits ou 64 bits) pour générer un package d’application Windows.
...
Cela peut se produire si les fichiers exécutables de votre application ont une extension **. EXE** en majuscules. Bien que la casse de cette extension ne doivent pas affecter si votre application s’exécute, cela peut entraîner la DAC générer cette erreur.

Pour résoudre ce problème, essayez de spécifier le **- AppExecutable** indicateur lorsque vous empaquetez et utilisez la minuscule « .exe » en tant que l’extension de votre fichier exécutable principal (par exemple : MYAPP.exe).    Alternativement, vous pouvez modifier la casse pour tous les exécutables dans votre application à partir de minuscules en majuscules (par exemple : à partir de. EXE .exe).
...
Pour plus d’informations, reportez-vous à la [spécification relative à l’exécutable portable Authenticode](https://download.microsoft.com/download/9/c/5/9c5b2167-8017-4bae-9fde-d599bac8184a/Authenticode_PE.docx) et à la [spécification relative au format de fichier PE](https://msdn.microsoft.com/windows/hardware/gg463119.aspx).
...
Après l’installation ou le lancement de certaines applications à partir du Microsoft Store, votre ordinateur peut redémarrer façon inattendue avec l’erreur : **0 x 139 (noyau\_sécurité\_vérifier\_ échec)**.
...
Si vous êtes développeur, vous ne voulez peut-être pas que votre application empaquetée soit installée sur des versions de Windows n’incluant pas cette mise à jour. Notez que ce faisant de votre application ne sera pas disponible pour les utilisateurs qui n’ont pas encore installé la mise à jour. Pour limiter la disponibilité de votre application aux utilisateurs qui ont installé cette mise à jour, modifiez votre fichier AppxManifest.xml comme suit :
...
### <a name="publisher-and-cert-mismatch-causes-signtool-error-error-signersign-failed--21470248850x8007000b"></a>Différence de serveur de publication et cert provoque l’erreur de Signtool « erreur : Échec de SignerSign() » (-2147024885/0x8007000b)
...
Exécutez **certutil** à partir de la ligne de commande sur le fichier PFX et copie le *sujet* champ à partir de la sortie.
...
Cela peut se produire lorsque votre package contient un fichier binaire qui dispose d’un certificat endommagé. Voici quelques-unes des raisons pourquoi cela peut se produire :

* Le début du certificat n’est pas à la fin d’une image.  

* La taille du certificat n’est pas positif.

* Le début du certificat n’est pas après le `IMAGE_NT_HEADERS32` structure d’un exécutable 32 bits ou après le `IMAGE_NT_HEADERS64` structure pour un exécutable 64 bits.

* Le pointeur de certificat n’est pas correctement aligné pour une structure WIN_CERTIFICATE.

Pour rechercher les fichiers qui contiennent un certificat PE incorrect, ouvrez un **invite de commandes**et définissez la variable d’environnement nommée `APPXSIP_LOG` sur une valeur de 1.

```
set APPXSIP_LOG=1
```

Ensuite, dans le **invite de commandes**, signer à nouveau votre application. Exemple :

```
signtool.exe sign /a /v /fd SHA256 /f APPX_TEST_0.pfx C:\Users\Contoso\Desktop\pe\VLC.appx
```

Informations sur les fichiers qui contiennent un certificat PE incorrect seront affiche dans le **fenêtre de Console**. Exemple :
