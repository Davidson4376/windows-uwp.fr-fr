...
description: Cette rubrique présente une étude de cas de portage d’une application Windows Phone Silverlight très simple à une application de plateforme Windows universelle (UWP) de Windows 10.
title: Windows Phone Silverlight à l’étude de cas UWP, Bookstore1
ms.date: 02/08/2017
ms.topic: article
...
ms.locfileid: "66372415"
---
# <a name="windowsphone-silverlight-to-uwp-case-study-bookstore1"></a>Silverlight de Windows Phone à l’étude de cas UWP : Bookstore1


Cette rubrique présente une étude de cas de portage d’une application Windows Phone Silverlight très simple à une application de plateforme Windows universelle (UWP) de Windows 10. Avec Windows 10, vous pouvez créer un package d’application unique vos clients peuvent installer sur un large éventail d’appareils, et c’est ce que nous allons faire dans cette étude de cas. Voir le [Guide des applications UWP](https://docs.microsoft.com/windows/uwp/get-started/universal-application-platform-guide).
...
**Remarque**    lors de l’ouverture Bookstore1Universal\_10 dans Visual Studio, si vous voyez le message « Visual Studio mise à jour requise », puis suivez les étapes permettant de sélectionner un contrôle de version de plateforme cible dans [ TargetPlatformVersion](wpsl-to-uwp-troubleshooting.md).

## <a name="downloads"></a>Téléchargements

[Télécharger le Windows Phone Bookstore1WPSL8 application Silverlight](https://go.microsoft.com/fwlink/?linkid=517053).
...
Modifier les fichiers de code et le balisage source que vous venez de copier et modifier toutes les références à l’espace de noms Bookstore1WPSL8 à Bookstore1Universal\_10. Une méthode rapide consiste à utiliser la fonctionnalité **Remplacer dans les fichiers**. Dans le code impératif du fichier source du modèle d’affichage, apportez les modifications de portage suivantes :
...
-   Modifier la valeur retournée par la **Bookstore1Universal\_10.BookstoreViewModel.AppName** propriété à partir de « BOOKSTORE1WPSL8 » à « BOOKSTORE1UNIVERSAL ».
...
Par défaut, toutes les orientations sont prises en charge. L’application Windows Phone Silverlight contraint explicitement lui-même en portrait uniquement, cependant, les éléments de dette dans ce \#1 et \#2 sont particulièrement porté ses en entrant dans le manifeste de package d’application dans le nouveau projet et en vérifiant **Portrait**sous **prise en charge des orientations**.

Pour cette application, élément \#3 n’est pas une dette, étant donné que la barre d’état (anciennement appelée la barre d’état système) s’affiche par défaut. Pour les éléments \#4 et \#5, nous devons trouver quatre Universal Windows Platform (UWP) **TextBlock** styles qui correspondent aux styles Windows Phone Silverlight que nous utilisions. Vous pouvez exécuter l’application Windows Phone Silverlight dans l’émulateur et les comparer côte à côte avec l’illustration dans le [texte](wpsl-to-uwp-porting-xaml-and-ui.md) section. À partir de cette méthode et l'on observe les propriétés des styles de système de Windows Phone Silverlight, nous pouvons faire cette table.
...
 
Pour définir ces styles, il vous suffit de les taper dans l’éditeur de balisage. Vous pouvez également utiliser les outils XAML de Visual Studio et les définir sans rien taper. Pour ce faire, vous cliquez sur un **TextBlock** et cliquez sur **modifier le Style** &gt; **appliquer la ressource**. Pour ce faire avec le **TextBlock**s dans le modèle d’élément, cliquez avec le bouton droit sur le **ListBox** et cliquez sur **modifier des modèles** &gt; **modifier Éléments (ItemTemplate) générés**.
