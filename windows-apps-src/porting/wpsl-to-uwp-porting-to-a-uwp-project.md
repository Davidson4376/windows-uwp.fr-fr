...
Si vous souhaitez prendre en charge à la fois WindowsPhone Silverlight et Windows 10 dans un seul fichier de code, puis vous pouvez le faire. Si vous regardez dans votre projet Windows 10 pages de propriétés, vous verrez que le projet définit WINDOWS\_UAP en tant que symbole de compilation conditionnelle. En règle générale, vous pouvez utiliser la logique suivante pour effectuer une compilation conditionnelle.
...
Si vous disposez de code partagé entre une application WindowsPhone Silverlight et une application de 8.x Windows Runtime, puis est peut-être déjà le code source avec une logique comme suit:
