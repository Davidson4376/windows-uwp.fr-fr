---
Description: Ce guide explique comment configurer votre Solution Visual Studio pour modifier, déboguer et créez un package d’application de bureau.
...
Vous pouvez utiliser Visual Studio pour générer un package de votre application de bureau. Ensuite, vous pouvez publier ce package pour le Microsoft Store ou le chargement de version test sur un ou plusieurs ordinateurs.
...
2. Ajoutez un **Projet de création de package d'application Windows** à votre solution.
   ...
   >[!NOTE]
   >Ce projet apparaît uniquement dans la version 15.5 ou supérieure de Visual Studio 2017.

3. Définissez la **Version cible** de ce projet sur la version de votre choix, mais veillez à définir la **Version minimum** sur **Mise à jour anniversaire Windows 10**.
...
6. Générez le projet de création de package pour vous assurer qu’aucune erreur n’apparaît.  Si vous recevez des erreurs, ouvrez **Configuration Manager** et vous assurer que vos projets ciblent la même plateforme.
...
7. Utilisez l'assistant [Créer des packages d'application](../packaging/packaging-uwp-apps.md) pour générer un fichier appxupload.
   ...
   Vous pouvez télécharger ce fichier directement dans le Store.
