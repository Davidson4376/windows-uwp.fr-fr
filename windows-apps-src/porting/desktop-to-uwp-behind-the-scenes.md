...
title: Fonctionnement détaillé de Desktop Bridge
ms.date: 05/25/2017
ms.topic: article
...
ms.locfileid: "57603644"
---
# <a name="behind-the-scenes-of-your-packaged-desktop-application"></a>Dans les coulisses de votre application de bureau empaquetée

Cet article fournit plus en détail ce qui se passe aux fichiers et entrées de Registre lorsque vous créez un package d’application Windows pour votre application de bureau.

Des principaux objectifs d’un package modern consiste à séparer l’état de l’application à partir de l’état du système autant que possible tout en conservant la compatibilité avec d’autres applications. Pour ce faire, le pont place l’application dans un package de plateforme Windows universelle (UWP), puis détecte et redirige certaines modifications qu’il apporte au système de fichiers et au Registre lors de l’exécution.

Les packages que vous créez pour votre application de bureau sont des applications de bureau uniquement, la confiance totale et ne sont pas virtualisés ou sandbox. Ainsi, ils peuvent interagir avec d’autres applications comme le font les applications de bureau classiques.

## <a name="installation"></a>Installation

Les packages d’application sont installés sous *C:\Program Files\WindowsApps\nom_package*, avec le fichier exécutable intitulé *app_name.exe*. Chaque dossier du package comporte un manifeste (nommé AppxManifest.xml) qui contient un espace de noms XML spécial pour les applications empaquetées. Ce fichier manifeste contient un élément ```<EntryPoint>``` qui fait référence à l’application totalement approuvée. Lorsque cette application est lancée, elle n’exécute pas à l’intérieur d’un conteneur d’application, mais au lieu de cela il exécute en tant que l’utilisateur comme il le ferait normalement.
...
Afin de pouvoir accueillir l’état de l’application, les modifications apportées par l’application à AppData sont capturées. Toutes les écritures effectuées dans le dossier AppData de l’utilisateur (par exemple, *C:\Users\user_name\AppData*), y compris la création, la suppression et la mise à jour, sont copiées au moment de l’écriture dans un emplacement privé par utilisateur, par application. Cette opération crée l’illusion que l’application empaquetée modifie AppData réel lorsque qu’elle modifie en fait une copie privée. En redirigeant les écritures de cette façon, le système peut suivre toutes les modifications de fichier effectuées par l’application. Cela permet au système nettoyer ces fichiers lorsque l’application est désinstallée, donc réduisant rot « système » et en fournissant une suppression d’une application une meilleure expérience pour l’utilisateur.

En plus de la redirection AppData, dossiers connus de Windows (System32, Program Files (x86), etc.) sont dynamiquement fusionnés avec les répertoires correspondants dans le package d’application. Chaque package contient un dossier nommé « VFS » à sa racine. Toutes les lectures de répertoires ou de fichiers dans le répertoire VFS sont fusionnées à l’exécution avec leurs équivalents natifs respectifs. Par exemple, une application peut contenir *C:\Program Files\WindowsApps\package_name\VFS\SystemX86\vc10.dll* comme partie de son package d’application, mais le fichier s’affiche pour être installée *C:\Windows\System32\ vc10.dll*.  Cela permet de préserver la compatibilité avec les applications de bureau qui s’attendent à trouver les fichiers dans d’autres emplacements que les packages eux-mêmes.
...
Le tableau suivant montre où les fichiers intégrés à votre package sont superposés sur le système de l’application. Votre application voit ces fichiers comme dans les emplacements système répertoriées, quand en fait, elles se trouvent dans les emplacements redirigés à l’intérieur de *C:\Program Files\WindowsApps\package_name\VFS*. Les emplacements FOLDERID proviennent des constantes [**KNOWNFOLDERID**](https://msdn.microsoft.com/library/windows/desktop/dd378457.aspx).

Emplacement système | Emplacement de redirection (sous [PackageRoot] \VFS\) | Valide sur les architectures
 :--- | :--- | :---
...
Seules les clés sous *HKLM\Software* font partie du package. Les touches sous *HKCU* ou d’autres parties du Registre n’en font pas partie. Les écritures dans les clés ou les valeurs du package ne sont pas autorisées. Écrit dans les clés ou valeurs pas partie du package sont autorisées en tant que l’utilisateur est autorisé.
...
Toutes les écritures sont conservés pendant la mise à niveau du package et supprimées uniquement lorsque l’application est entièrement supprimée.
...
Lorsqu’un package est désinstallé par l’utilisateur, tous les fichiers et dossiers situés sous *C:\Program Files\WindowsApps\package_name* sont supprimées, ainsi que toutes les écritures redirigés dans AppData ou du Registre qui ont été capturés au cours de la processus d’empaquetage.
