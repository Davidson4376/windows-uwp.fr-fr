...
Pour implémenter une file d’attente de notifications, commencez par activer la file d’attente pour votre vignette (voir [Utilisation de la file d’attente de notifications avec des notifications locales](https://msdn.microsoft.com/library/windows/apps/hh465429)). L’appel qui permet d’activer la file d’attente ne doit être effectué qu’une seule et unique fois. Toutefois, cela ne pose aucun problème d’effectuer l’appel chaque fois que votre application est lancée.
