...
Si vous préférez utiliser du code XML brut plutôt que la [bibliothèque Notifications](https://www.nuget.org/packages/Microsoft.Toolkit.Uwp.Notifications/), consultez la rubrique [schéma XML](tiles-and-notifications-toast-xml-schema.md).
...
|---|---|---|---|
| **Text** | chaîne ou BindableString | false | Le texte à afficher. Prise en charge de la liaison de données ajoutée dans Creators Update, mais fonctionne uniquement pour les éléments de texte de premier niveau. |
...
| Propriété | Type | Obligatoire | Description |
|---|---|---|---|
| **url** | uri | false | Le fichier multimédia à lire à la place du son par défaut. Seuls ms-appx et ms-appdata sont pris en charge. |
...
Un en-tête personnalisé qui regroupe plusieurs notifications dans le centre de notifications.
...
| **action**| Action | false | Déclare l’action à effectuer lorsque l’utilisateur clique sur la notification toast. |
