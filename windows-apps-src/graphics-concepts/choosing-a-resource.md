...
## <a name="span-ididentifybindingspanspan-ididentifybindingspanspan-ididentifybindingspanidentify-pipeline-stages-that-need-resources"></a><span id="Identify_Binding"></span><span id="identify_binding"></span><span id="IDENTIFY_BINDING"></span>Identifier les étapes de canalisation ont besoin de ressources
...
| Étape du pipeline  | Entrée/Sortie | Resource               | Type de ressource                           |
|-----------------|--------|------------------------|-----------------------------------------|
| Assembleur d’entrée | Vers l’intérieur     | Mémoire tampon de vertex          | Mémoire tampon                                  |
| Assembleur d’entrée | Vers l’intérieur     | Mémoire tampon d’index           | Mémoire tampon                                  |
| Étapes du nuanceur   | Vers l’intérieur     | Shader-ResourceView    | Mémoire tampon, Texture1D, Texture2D, Texture3D |
| Étapes du nuanceur   | Vers l’intérieur     | Mémoire tampon constante du nuanceur | Mémoire tampon                                  |
| Sortie de flux   | Vers l’extérieur    | Mémoire tampon                 | Mémoire tampon                                  |
| Fusion de sortie   | Vers l’extérieur    | Affichage de la cible de rendu     | Mémoire tampon, Texture1D, Texture2D, Texture3D |
| Fusion de sortie   | Vers l’extérieur    | Affichage du gabarit de profondeur     | Texture1D, Texture2D                    |

 

## <a name="span-ididentifyusagespanspan-ididentifyusagespanspan-ididentifyusagespanidentify-how-each-resource-will-be-used"></a><span id="Identify_Usage"></span><span id="identify_usage"></span><span id="IDENTIFY_USAGE"></span>Identifier comment chaque ressource doit être utilisée
...
|----------------|--------------------------------------|---------------------|
| Default        | Processeur graphique                                  | rarement        |
| Mobile        | Processeur                                  | fréquemment          |
| Création intermédiaire        | Processeur graphique                                  | Non applicable                 |
...
| L'application remplit les ressources à plusieurs reprises | Mobile              |
| Rendu de texture                     | Default              |
| Accès des données du processeur graphique par l'UC                | Création intermédiaire              |
...
## <a name="span-idresourcetypesandpipelinestagesspanspan-idresourcetypesandpipelinestagesspanspan-idresourcetypesandpipelinestagesspanbinding-resources-to-pipeline-stages"></a><span id="Resource_Types_and_Pipeline_stages"></span><span id="resource_types_and_pipeline_stages"></span><span id="RESOURCE_TYPES_AND_PIPELINE_STAGES"></span>Liaison des ressources à des étapes de canalisation
