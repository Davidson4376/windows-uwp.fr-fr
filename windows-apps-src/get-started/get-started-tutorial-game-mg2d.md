---
title: Didacticiel de prise en main- Jeu UWP en MonoGame2D
description: "Un jeu UWP simple pour le Windows Store, écrit en C# et MonoGame"
author: mukin
ms.author: mukin
ms.date: 03/06/2017
ms.topic: article
...
translationtype: HT
---
# <a name="get-started-tutorial-a-uwp-game-in-monogame-2d"></a>Didacticiel de prise en main: jeu UWP en MonoGame2D

## <a name="a-simple-2d-uwp-game-for-the-windows-store-written-in-c-and-monogame"></a>Un jeu UWP simple en2D pour le Windows Store, écrit en C# et MonoGame
...
+    Windows10 et Microsoft Visual Studio 2015. [Cliquez ici pour savoir comment prendre en main VisualStudio](https://docs.microsoft.com/en-us/windows/uwp/get-started/get-set-up).
...
Ouvrez le projet dans Visual Studio2015, puis appuyez sur **F5** pour exécuter l’exemple. Cela peut prendre un certain temps la première fois, puisque Visual Studio doit extraire les packages NuGet qui peuvent manquer dans votre installation.
...
**Remarque:** le jeu créé dans cet exemple ne vise pas à être complet (ni même amusant). Son seul objet est d'illustrer l'ensemble des concepts principaux du développement2D dans MonoGame. N’hésitez pas à utiliser ce code pour créer quelque chose de plus intéressant ou même repartir de zéro lorsque vous maîtriserez les concepts de base!
...
Pour l’avatar du joueur, nous allons utiliser le Chat ninja de Microsoft lui-même, chevauchant son fidèle T-Rex. [Cliquez ici pour télécharger l’image](https://github.com/muhsinking/VeggieJump/blob/master/MonoGame2D/Content/ninja-cat-dino.png).
...
7. Gestion des entrées clavier: nous avons ensuite besoin d’une nouvelle méthode pour gérer ce que l'utilisateur saisit au clavier. Ajoutez cette méthode à **Game1.cs**:
...
## <a name="publish-to-the-windows-store"></a>Publication dans le Windows Store
Puisque nous avons créé ce jeu sous forme d'application UWP, nous pouvons publier le projet dans le Windows Store. La procédure nécessite quelques étapes.
...
Pour plus d’informations, voir [Publication de votre application du Windows Store](https://developer.microsoft.com/en-us/store/publish-apps).
