---
title: Créer un jeu UWP dans MonoGame 2D
...
ms.locfileid: "64564530"
---
# <a name="create-a-uwp-game-in-monogame-2d"></a>Créer un jeu UWP dans MonoGame 2D

## <a name="a-simple-2d-uwp-game-for-the-microsoft-store-written-in-c-and-monogame"></a>Un jeu UWP simple en 2D pour le Microsoft Store, écrit en C# et MonoGame


![Feuille de sprite Dino marcheur](images/JS2D_0.png)
...
Ouvrez le projet dans Visual Studio 2019, puis appuyez sur **F5** pour exécuter l’exemple. Cela peut prendre un certain temps la première fois, puisque Visual Studio doit extraire les packages NuGet qui peuvent manquer dans votre installation.
...
2. Démarrez Visual Studio 2019.

3. Accédez à **Fichier -> Nouveau -> Projet**

4. Sous les modèles de projet Visual C#, sélectionnez **MonoGame** et **Projet MonoGame Universal Windows 10**

5. Nommez votre projet « MonoGame2D » et sélectionnez OK. Une fois créé, le projet aura probablement l’air truffé d’erreurs. Ces dernières devraient disparaître lorsque vous aurez exécuté le projet pour la première fois et que tous les packages NuGet manquants auront été installés.

6. Assurez-vous que **x86** et **Local Machine** sont définis comme plateforme cible, puis appuyez sur **F5** pour générer et exécuter le projet vide. Si vous avez suivi les étapes ci-dessus, une fenêtre bleue vide doit apparaître une fois que le projet a achevé la génération.

## <a name="method-overview"></a>Vue d’ensemble de la méthode
Maintenant que vous avez créé le projet, ouvrez le fichier **Game1.cs** à partir de l’**Explorateur de solutions**. C’est là qu’ira la majeure partie de la logique du jeu. De nombreuses méthodes cruciales sont générées automatiquement ici lorsque vous créez un projet MonoGame. Passons-les rapidement en revue :

**public Game1()** Constructeur. Nous ne modifierons pas du tout cette méthode dans ce didacticiel.
...
**protected override void LoadContent()** Cette méthode charge le contenu (par exemple, textures, sons, polices) dans la mémoire avant le démarrage du jeu. Comme Initialize, elle est appelée une fois lorsque l’application démarre.
...
**protected override void Update(GameTime gameTime)** Cette méthode est appelée une fois à chaque cycle de la boucle de jeu. Ici, nous mettons à jour les états de n’importe quel objet ou de n’importe quelle variable dans le jeu. Cela inclut des éléments tels que la position d’un objet, sa vitesse ou sa couleur. C’est également là que les entrées de l’utilisateur sont gérées. Bref, cette méthode traite chaque partie de la logique du jeu à l’exception du dessin des objets à l’écran.
...
### <a name="3-add-class-variables"></a>3. Ajout de variables de classe
...
Avant de commencer à animer les sprites, nous allons créer une classe appelée « SpriteClass » qui nous permettra de simplifier la manipulation des sprites au niveau de la surface.

### <a name="1-create-a-new-class"></a>1. Création d’une classe
...
Maintenant que la SpriteClass est créée, nous allons l’utiliser pour créer deux objets du jeu. Le premier est un avatar que le joueur peut contrôler avec les touches de direction et la barre d’espace. Le second est un objet que le joueur doit éviter.
...
Exactement comme pour le rectangle vert, ajoutez ces images à **Content.mgcb** par le biais du **Pipeline MonoGame** et appelez-les respectivement « ninja-cat-dino.png » et « broccoli.png ».
...
**spaceDown** suit la position de la barre d’espace pour déterminer si elle maintenue enfoncée ou si elle est enfoncée puis relâchée.

**gameStarted** nous prévient si c’est la première fois que l’utilisateur entre dans le jeu.
...
**dinoSpeedX** et **dinoJumpY** déterminent la vitesse à laquelle l’avatar du joueur se déplace et saute.
score compte les obstacles que le joueur a correctement évités.

Enfin, **random** servit à ajouter une dose de hasard au comportement de l’obstacle brocoli.
...
Les trois dernières variables doivent être mises à l’échelle pour les périphériques haute résolution, car elles spécifient un taux de changement en pixels.
...
La taille de l’image du brocoli est nettement plus grande que celle que nous voulons lui donner dans le jeu. Nous allons donc la ramener à 0,2 fois sa taille d’origine.

### <a name="5-program-obstacle-behaviour"></a>5. Programmation du comportement d’un obstacle
Nous voulons que le brocoli soit généré quelque part hors de l’écran et aille en direction de l’avatar du joueur, qui doit alors les éviter. Pour cela, ajoutez cette méthode à la classe **Game1.cs** :
...
La deuxième partie détermine la vitesse à laquelle le brocoli se déplace, ce qui est déterminé par le score du moment. Il accélère chaque fois que le joueur réussit à en éviter cinq.
...
### <a name="7-handle-keyboard-input"></a>7. Gestion des entrées du clavier
Ensuite, nous avons besoin d’une nouvelle méthode pour traiter les entrées de l’utilisateur par le biais du clavier. Ajoutez cette méthode à **Game1.cs** :
...
La deuxième démarre le jeu si la touche **Espace** est enfoncée et si le jeu n’a pas déjà démarré.

La troisième fait sauter l’avatar dino si la touche **Espace** est enfoncée, en modifiant sa propriété **dY**. Le joueur ne peut sauter que s’il se trouve sur le « sol » (dino.y = screenHeight * SKYRATIO). Il ne peut pas non plus sauter si la touche espace est maintenue enfoncée au lieu d’être enfoncée une seule fois. Cela empêche le dino de sauter dès que le jeu démarre, puisqu’il utilise la même touche que celle qui démarre le jeu.
...
En utilisant le code ci-dessus, nous suivons le score du joueur en arrière-plan, mais nous ne l’indiquons pas au joueur en réalité. Nous avons également une introduction relativement peu intuitive au démarrage de l’application : le joueur voit une fenêtre bleue et verte, mais il n’a aucun moyen de savoir qu’il doit appuyer sur espace pour démarrer le jeu.
...
Dans **Score.spritefont**, modifiez la valeur **<Size>** de 12 à 36.

Dans **GameState.spritefont**, modifiez la valeur **<Size>** de 12 à 72 et la valeur **<FontName>** d’Arial à Agency. Agency est une autre police livrée en standard avec les ordinateurs Windows 10. Elle ajoute une certaine élégance à notre écran de présentation.

### <a name="3-load-spritefonts"></a>3. Chargement de SpriteFonts
De retour dans Visual Studio, nous allons tout d’abord ajouter une texture pour l’écran de présentation de Microsoft au démarrage. [Cliquez ici pour télécharger l’image](https://github.com/Microsoft/Windows-appsample-get-started-mg2d/blob/master/MonoGame2D/Content/start-splash.png).
...
Le code ci-dessus utilise la description de sprite que nous avons créée (Arial taille 36) pour tirer le score actuel du joueur vers le coin supérieur droit de l’écran.
...
Nous avons un brocoli qui vous suit partout, ainsi qu’un score qui augmente chaque fois qu’un brocoli est créé, mais pour l’instant, il n’existe aucun moyen de perdre réellement à ce jeu. Il nous faut un moyen de savoir si les sprites dino et brocoli entrent en collision et, quand c’est le cas, de déclarer que le jeu est fini.

### <a name="1-get-the-textures"></a>1. Obtention des textures
La dernière image dont nous avons besoin représente la fin de jeu. [Cliquez ici pour télécharger l’image](https://github.com/Microsoft/Windows-appsample-get-started-mg2d/blob/master/MonoGame2D/Content/game-over.png).

Exactement comme pour le rectangle vert, ninja-cat et broccoli, ajoutez cette image à **Content.mgcb** par le biais du **Pipeline MonoGame** et appelez-la « game-over.png ».

### <a name="2-rectangular-collision"></a>2. Collision rectangulaire
Lorsque des collisions sont détectées dans un jeu, les objets sont souvent simplifiés en vue de réduire la complexité des calculs nécessaires. Nous allons traiter à la fois l’avatar du joueur et l’obstacle brocoli comme des rectangles afin de détecter leurs collisions.

Ouvrez **SpriteClass.cs** et ajoutez une variable de classe :
...
Ajoutez ensuite une méthode pour **SpriteClass.cs** :
...
Ouvrez ensuite **Game1.cs** et ajoutez deux variables de classe, l’une pour stocker la texture du sprite signifiant la fin du jeu et une valeur booléenne pour suivre l’état du jeu :
...
Initialisez ensuite **gameOver** dans la méthode **Initialize** :

```CSharp
gameOver = false;
```

Enfin, chargez la texture dans **gameOverTexture** au sein de la méthode **LoadContent** :

```CSharp
gameOverTexture = Content.Load<Texture2D>("game-over");
```

### <a name="4-implement-game-over-logic"></a>4. Implémentation de la logique « fin du jeu »
...
### <a name="5-add-user-input-for-resetting-the-game"></a>5. Ajout de l’entrée utilisateur permettant de réinitialiser le jeu
...
### <a name="6-draw-game-over-splash-and-text"></a>6. Dessin de l’écran et du texte de fin de jeu
...
Nous utilisons ici la même méthode que celle employée pour tracer le texte centré à l’horizontale (en réutilisant la police de présentation de démarrage) et pour centrer **gameOverTexture** dans la partie supérieure de la fenêtre.
...
Pour plus d’informations, voir [Publication de votre application UWP](https://developer.microsoft.com/en-us/store/publish-apps).
