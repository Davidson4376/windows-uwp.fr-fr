...
**Créer une application Web hébergée pour le Microsoft Store avec fullstack populaires technologies web**
...
Si vous ne disposez pas encore de ces ressources sur votre ordinateur, suivez ces liens de téléchargement :
 ...
 - [Générateur Express](https://expressjs.com/en/starter/generator.html)- après l’installation de nœud, installez Express en exécutant `npm install express-generator -g`
...
## <a name="part-i-build-a-rest-api-backend"></a>1ère partie : Générer un serveur principal d’API REST
...
Si vous souhaitez ignorer cette partie et passer directement à [2ème partie : Créer une application web de page unique](#part-ii-build-a-single-page-web-application), voici la [terminé de code pour la partie I](https://github.com/Microsoft/Windows-tutorials-web/tree/master/Single-Page-App-with-REST-API/backend). Suivez le *Lisez-moi* obtenir des instructions pour obtenir le code d’et en cours d’exécution localement, ou voir *5. Héberger votre service d’API sur Azure et activer CORS* pour l’exécuter à partir d’Azure.
...
### <a name="1-spec-out-the-api-and-generate-code-stubs"></a>1. Spécification de l’API et générer des stubs de code
...
### <a name="2-customize-the-server-code-and-setup-debugging"></a>2. Personnaliser le code serveur et le programme d’installation de débogage
...
### <a name="3-set-up-your-route-handlers"></a>3. Configurez vos gestionnaires d’itinéraire
...
### <a name="5-optional-host-your-api-service-on-azure-and-enable-cors"></a>5. (Facultatif) Héberger votre service d’API sur Azure et activer CORS

La documentation Azure vous guidera à travers les étapes suivantes :

 - [Inscription d’un nouveau *application API* avec le portail Azure](https://docs.microsoft.com/azure/app-service/app-service-web-tutorial-rest-api#createapiapp)
 ...
 - [Déploiement de votre code d’application API dans Azure](https://docs.microsoft.com/azure/app-service/app-service-web-tutorial-rest-api#deploy-the-api-with-git)
...
Voici quelques ressources utiles pour aller plus loin :
 ...
 - [Node.js avancées de débogage avec Visual Studio Code](https://code.visualstudio.com/docs/nodejs/nodejs-debugging)
...
## <a name="part-ii-build-a-single-page-web-application"></a>Partie II : Créer une application web de page unique
...
Ici, nous utilisons [ `jQuery.ajax()` ](https://api.jquery.com/jQuery.ajax/) et **PUT** [ `/guess` ](#part-i-build-a-rest-api-backend) méthode créée dans la partie I. 

Ce code s’exécute dans l’ordre suivant.

* Le `id` de la première carte sélectionnée par l’utilisateur est ajouté en tant que la première valeur dans le tableau de [] selectedCards : `selectedCards[0]` 
...
* Un [glyphicon Bootstrap](https://getbootstrap.com/components/) est ajouté à l’arrière de la carte dont `id` est `selectedCards[0]`
...
> [!TIP] 
> N’oubliez pas : Pug est un espace blanc sensible. Assurez-vous que toutes vos mises en retrait sont correctes !

### <a name="4-use-bootstraps-grid-system-to-create-a-responsive-layout"></a>4. Utiliser le système de grille de Bootstrap pour créer une disposition réactive
Le [système de grille](https://getbootstrap.com/css/#grid) de Bootstrap est un système de grille fluide qui s’adapte à une grille au fil des modifications de la fenêtre d’affichage d’un appareil. Les cartes dans ce jeu utilisent des classes de système de grille Bootstrap prédéfinies pour la disposition, notamment :
* `.container-fluid`: Spécifie le conteneur fluide pour la grille
* `.row-fluid`: Spécifie les lignes FLUIDE
...
### <a name="5-add-a-card-flip-animation-with-css-transforms"></a>5. Ajouter une animation de retournement de la carte avec CSS transforme
...
2. Maintenant, ajoutons les propriétés suivantes à la classe `.cards` dans style.css. Le `.cards` `div` est l’élément qui effectuera réellement l’animation retournement, montrant l’avant ou l’arrière de la carte. 
...
3.  À l’aide de la propriété [`transform`](https://developer.mozilla.org/en-US/docs/Web/CSS/transform), nous pouvons faire pivoter la carte autour de l’axe Y.  Ajoutez le CSS suivant à `cards.flip`.
    ...
    Désormais, lorsqu’un utilisateur clique sur une carte, celle-ci pivote à 180 degrés.

### <a name="6-test-and-play"></a>6. Test- and -play
Félicitations ! Vous avez terminé de créer l’application Web ! Testons-la. 

1. Ouvrez une invite de commandes dans votre répertoire de mémoire et entrez la commande suivante : `npm start`
...
Vous pouvez désormais déployer votre application Azure (ou tout autre service d’hébergement dans le cloud) en vue de tests sur divers facteurs de forme d'appareils, tels que des appareils mobiles, tablettes et bureau. (Ne pas avoir oublié tester avec différents navigateurs trop !) Une fois que votre application est prête pour la production, vous pouvez empaqueter facilement en tant qu’un *application Web hébergée* (HWA) pour le *plateforme Windows universelle* (UWP) et la distribuer à partir du Microsoft Store.
...
Voici quelques ressources utiles pour aller plus loin :
 ...
 - [Déployer votre projet de développement d’application sur sites Web Azure](https://docs.microsoft.com/azure/cosmos-db/documentdb-nodejs-application#_Toc395783182)

 - [Convertir votre application web à une application de plateforme universelle Windows (UWP)](https://docs.microsoft.com/en-us/windows/uwp/porting/hwa-create-windows)
