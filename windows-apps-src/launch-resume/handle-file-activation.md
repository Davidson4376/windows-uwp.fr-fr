...
> [!NOTE]
> Une fois lancé via le contrat de fichier, assurez-vous que ce bouton Précédent ramène l’utilisateur à l’écran qui a lancé l’application et pas au contenu précédent de l’application.

Nous vous recommandons de créer un nouveau XAML **Frame** pour chaque événement d’activation qui ouvre une nouvelle page. De cette façon, le backstack de navigation pour le nouveau Frame XAML ne contient pas n’importe quel contenu précédente que l’application peut-être avoir sur la fenêtre active lorsque suspendu. Si vous décidez d’utiliser un seul XAML **Frame** pour le lancement et pour les contrats de fichier, vous devez effacer les pages dans le **Frame**du journal de navigation avant de naviguer vers une nouvelle page.

Lorsque votre application est lancée par le biais de l’activation du fichier, vous devez envisager, notamment l’interface utilisateur qui permet à l’utilisateur revenir à la première page de l’application.
...
* [Exemple de lancement association](https://go.microsoft.com/fwlink/p/?LinkID=231484)
