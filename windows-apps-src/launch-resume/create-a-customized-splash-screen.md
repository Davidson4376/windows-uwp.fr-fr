...
> **Remarque**l’expression «écran de démarrage étendu» dans cette rubrique fait référence à un écran de démarrage qui reste à l’écran pendant une période prolongée. Elle ne désigne pas une sous-classe qui dérive de la classe [**SplashScreen**](https://msdn.microsoft.com/library/windows/apps/br224763).
...
-   La page de votre écran de démarrage étendu doit utiliser une image de 620x300 pixels cohérente avec l’image spécifiée pour l’écran de démarrage dans le manifeste de votre application (image de l’écran de démarrage de votre application). Dans Studio2015 visuelle de Microsoft, les paramètres d’écran de démarrage sont stockés dans la section de **l’Écran de démarrage** de l’onglet **Ressources visuelles** dans votre manifeste d’application (fichier Package.appxmanifest).
...
-   Ouvrez votre application dans Visual Studio2015.
...
App.Xaml.cs: Ce projet a été créé à l’aide du modèle de projet **Application vide (XAML)** application UWP dans Visual Studio2015. Les gestionnaires d’événements `OnNavigationFailed` et `OnSuspending` sont générés automatiquement et n’ont pas besoin d’être modifiés pour l’implémentation d’un écran de démarrage étendu. Cette rubrique modifie uniquement `OnLaunched`.
