...
* **Ne pas créer des activités pour les actions effectuées par les autres utilisateurs.** Si un compte externe envoie un message à l’utilisateur ou @-mentions leur au sein de votre application, vous ne devez pas créer une activité pour ce faire. Ce type d’action est mieux servi par des Notifications du centre.
...
* **Jeux** : créer une activité pour chaque jeu enregistrer ou le monde. Si votre jeu prend en charge uniquement une séquence de niveaux, vous pouvez republiez la même activité au fil du temps, bien que vous pouvez mettre à jour les données de contenu pour afficher la progression ou les succès dernières.
