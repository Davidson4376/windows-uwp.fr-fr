...
* **Ne pas créer des activités pour les actions effectuées par les autres utilisateurs.** Si un compte externe envoie un message à l’utilisateur ou @-mentions leur au sein de votre application, vous ne devez pas créer une activité pour cela. Ce type d’action est mieux servi par des Notifications du centre.
...
* **Applications basées sur le document** : créer une activité par document, avec un ou plusieurs éléments de l’historique reflétant les périodes d’utilisation. Il est important de mettre à jour votre activité lorsque des modifications sont apportées au document.
...
* Utiliser des images
  ...
  * Si l’image ne peut pas être rognée sur 2:1, vous devez le placer dans la carte d’activité.  
