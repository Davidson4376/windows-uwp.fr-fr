...
description: "Ce livre blanc décrit les techniques de migration des applications .NET et Win32 depuis des ressources Win32héritées à des ressources MRT lorsqu'elles sont emballées en tant que packages AppX."
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>Migrer les ressources héritées vers la plateforme de gestion de ressources de Windows10

## <a name="executive-summary"></a>Résumé

Les applications Win32 sont souvent traduites dans différentes langues, agrandissant ainsi leur marché global<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>. Il existe plusieurs façons de localiser les applications Win32traditionnelles, mais Windows8 a introduit un [nouveau système de gestion des ressources](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx) (appelé «MRT» dans ce document <a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>) qui fonctionne sur l'ensemble des langages de programmation et des types d’applications et fournit des fonctionnalités supplémentaires et allant au-delà de la simple localisation. 

Combiné à un déploiement basé sur les AppX (par ex., à partir du Windows Store), MRT peut fournir automatiquement les ressources plus applicables à un utilisateur ou périphérique donné, ce qui réduit la taille de téléchargement et d'installation de votre application. Cette réduction de la taille peut être significative pour les applications contenant une grande quantité de contenu localisé, et peut relever de l'ordre de plusieurs *gigaoctets* pour les jeux AAA. MRT confère des avantages supplémentaires, comme des listes localisées dans l’interpréteur de commandes Windows et le Windows Store, la logique de secours automatique lorsque la langue par défaut d’un utilisateur ne correspond pas à vos ressources disponibles.
...
<td>Travail minimal requis pour que votre contenu localisé apparaisse dans l’interpréteur de commandes Windows et dans le Windows Store</td>
<td>Petite</td>
</tr>
<tr>
...
Voici un exemple simple d’une application dotée d'étiquettes de texte sur deux boutons (`openButton` et `saveButton`) et d'un fichier PNG utilisé pour un logo (`logoImage`). Les étiquettes de texte sont localisées en anglais et en allemand et le logo est optimisé pour les écrans de bureau normaux (facteur d’échelle de 100%) et les téléphones haute résolution (facteur d’échelle de 300%) <a name="footnote4_ref" href="#footnote4"><sup>4</sup></a>. Veuillez noter que le schéma suivant présente une vue conceptuelle de haut niveau du modèle; il ne correspond pas exactement à l'implémentation:
...
Après avoir effectué les phases de 0 à 3 susmentionnées, vous disposerez d'un «lot» d'application, qui pourra être soumis au Windows Store et qui réduira les tailles de téléchargement et d'installation pour les utilisateurs, en omettant les ressources dont ces derniers n'ont pas besoin (par ex., les langues qu'ils ne parlent pas). Vous pouvez apporter des améliorations supplémentaires au niveau de la taille et des fonctionnalités de l’application en effectuant une étape finale. 
...
0. Dans le fichier XML, ajoutez le contenu suivant, le texte <span style="background-color: yellow">mis en surbrillance</span> étant remplacé par le texte correspondant à votre application, dans votre langue par défaut<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>
...
Si vous modifiez directement le fichier XML, ouvrez le fichier `AppxManifest.xml` et apportez les modifications suivantes aux valeurs <span style="background-color: lightgreen"> mises en surbrillance</span>; utilisez *exactement* ce texte, et non pas un texte spécifique à votre application<a name="footnote6_ref" href="#footnote6"><sup>6</sup></a>. Ces noms doivent correspondre aux `Names` que vous avez créés dans le fichier `.resw`, avec le préfixe `ms-resource:` et l'espace de noms `Resources/`. 
...
À titre de contre-exemple, si votre application utilise une structure de fichiers plate avec un seul répertoire `Resources` contenant les fichiers `english_strings.dll` et `french_strings.dll`, elle ne sera ** pas** mappée correctement sur MRT. Une meilleure structure serait un répertoire `Resources` avec des fichiers et des sous-répertoires `en\strings.dll` et `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a>. Notez qu’il est toujours possible d’utiliser MRT et les avantages de la création de packages AppX, même si vous ne pouvez pas suivre cette convention d’affectation de noms de fichier; cela nécessite simplement plus de travail. Un livre blanc ultérieur abordera ce cas. 
...
L'exécution .NET déclenchera l'événement `AssemblyResolve` chaque fois qu’elle ne parvient pas à trouver les DLL de ressource; à ce moment-là, le gestionnaire d’événement localisera le fichier souhaité via MRT et retournera l’assembly de ressource<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>.
...
Cliquez sur le nombre pour revenir à l’emplacement source dans le document.

<a name="footnote1" href="#footnote1_ref">1.</a> Ce livre blanc ne fournit aucune motivation supplémentaire pour la localisation proprement dite. Il est supposé que le lecteur comprend cela.

<a name="footnote2" href="#footnote2_ref">2.</a> Historiquement cela signifiait «Modern Resource Technology», mais le terme «Modern» a été abandonné. Le gestionnaire de ressources peut également être appelé MRM (Gestionnaire de ressources modernes) ou PRI (Index de ressource de package).

<a name="footnote3" href="#footnote3_ref">3.</a> Ce tableau n’inclut pas les tâches non liées à la localisation, par exemple des icônes d’application haute résolution ou à contraste élevé. Pour plus d’informations sur la fourniture de plusieurs ressources pour les vignettes, les icônes, etc., consultez MSDN.

<a name="footnote4" href="#footnote4_ref">4.</a> Les spécificités de facteur d’échelle ne sont pas abordées ici

<a name="footnote5" href="#footnote5_ref">5.</a> Notez qu’il existe des restrictions sur les longueurs de certaines de ces chaînes; voir [la rubrique **VisualElements** sur MSDN](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx) pour plus d’informations 

<a name="footnote6" href="#footnote6_ref">6.</a> Il n’est pas obligatoire d’utiliser ces noms de ressource exacts, vous pouvez choisir le vôtre, mais tout ce que vous choisissez doit correspondre exactement à l'ensemble du contenu du fichier `.resw`.

<a name="footnote7" href="#footnote7_ref">7.</a> Vous pouvez choisir n’importe quel autre répertoire de votre choix, mais veillez à remplacer cela dans les commandes futures

<a name="footnote8" href="#footnote8_ref">8.</a> Il est également possible d’utiliser le même nom de fichier de base mais avec des qualificateurs incorporés, tel que `strings.lang-en.dll` et `strings.lang-fr.dll`, mais l'utilisation de répertoires avec des codes de langue est plus simple sur le plan conceptuel, aussi nous allons nous concentrer sur cette méthode.

<a name="footnote9" href="#footnote9_ref">9.</a> Notez que si votre application possède déjà un gestionnaire  `AssemblyResolve` à d’autres fins, vous devez intégrer le code de résolution des ressources dans votre code existant.
