...
* Si vous disposez d’une application de bureau Win32 de C/C++ et que vous souhaitez pour héberger des contrôles UWP dans votre application, vous devez utiliser l’API d’hébergement de XAML UWP. Il n’existe aucune alternative pour ces types d’applications.
...
La manière d’utiliser le XAML UWP hébergeant des API dans votre code dépend de votre type d’application, la conception de votre application et d’autres facteurs. Pour illustrer comment utiliser cette API dans le contexte d’une application complète, cet article fait référence au code dans les exemples suivants.
  ...
  * **C/C++ Win32:** L’exemple [XamlIslands32](https://github.com/clarkezone/cppwinrt/tree/master/Desktop/XamlIslandsWin32) sur GitHub. Cet exemple montre comment utiliser le XAML UWP hébergeant des API dans une application Win32 simple et de gérer les modifications PPP.

  * **WPF:** La version WPF du contrôle [**WindowsXamlHost**](https://github.com/Microsoft/WindowsCommunityToolkit/tree/master/Microsoft.Toolkit.Win32/Microsoft.Toolkit.Wpf.UI.XamlHost) dans le Kit de ressources de la Communauté Windows. Ce contrôle est dérivé de [**System.Windows.Interop.HwndHost**](https://docs.microsoft.com/dotnet/api/system.windows.interop.hwndhost.aspx) et utilise le XAML UWP, API d’hébergement en interne. Pour les informations générales sur l’utilisation de **System.Windows.Interop.HwndHost** dans une application WPF, consultez [Hébergement de contenu Win32 dans WPF](https://docs.microsoft.com/dotnet/framework/wpf/advanced/hosting-win32-content-in-wpf).

  * **Windows Forms:** La version Windows Forms du contrôle [**WindowsXamlHost**](https://github.com/Microsoft/WindowsCommunityToolkit/tree/master/Microsoft.Toolkit.Win32/Microsoft.Toolkit.Forms.UI.XamlHost) dans le Kit de ressources de la Communauté Windows. Ce contrôle est dérivé de [**System.Windows.Forms.Control**](https://docs.microsoft.com/dotnet/api/system.windows.forms.control) et utilise le XAML UWP, API d’hébergement en interne.
...
* Pour utiliser le XAML UWP hébergeant des API dans votre application de bureau, vous devez configurer votre projet, vous pouvez appeler des API UWP:

    * **C/C++ Win32:** Nous vous recommandons de configurer votre projet pour utiliser [C++ / WinRT](../cpp-and-winrt-apis/index.md). Téléchargez et installez le [C++ / Extension WinRT Visual Studio (VSIX)](https://aka.ms/cppwinrt/vsix) à partir de la place de marché Microsoft Visual Studio, puis ajoutez le ```<CppWinRTEnabled>true</CppWinRTEnabled>``` propriété à votre fichier .vcxproj comme décrit [ici](../cpp-and-winrt-apis/intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-and-the-vsix).
...
* Au niveau de base est l’élément d’interface utilisateur dans votre application dans laquelle vous souhaitez héberger l’île XAML. Cet élément d’interface utilisateur doit avoir un handle de fenêtre (HWND). Les éléments d’interface utilisateur dans laquelle vous pouvez héberger une île XAML incluent [**System.Windows.Interop.HwndHost**](https://docs.microsoft.com/dotnet/api/system.windows.interop.hwndhost) pour les applications WPF, [**System.Windows.Forms.Control**](https://docs.microsoft.com/dotnet/api/system.windows.forms.control) pour les applications Windows Forms et une [fenêtre](https://docs.microsoft.com/windows/desktop/winmsg/about-windows) pour les applications Win32 de C/C++.
...
2. Créer un objet [**DesktopWindowXamlSource**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.desktopwindowxamlsource) et l’associer à un élément d’interface utilisateur dans votre application qui est associé à un handle de fenêtre parente.

    Pour ce faire, vous devrez procédez comme suit:

    1. Créer un objet **DesktopWindowXamlSource** et effectuer un cast de l’interface **IDesktopWindowXamlSourceNative** COM. Cette interface est déclarée dans le ```windows.ui.xaml.hosting.desktopwindowxamlsource.h``` fichier d’en-tête dans le SDK Windows. Dans un projet Win32 de C/C++, vous pouvez référencer directement ce fichier d’en-tête. Dans un projet WPF ou Windows Forms, vous devez déclarer cette interface dans le code de votre application à l’aide de l’attribut [**ComImport**](https://docs.microsoft.com/dotnet/api/system.runtime.interopservices.comimportattribute) . Assurez-vous que votre déclaration de l’interface correspond exactement à la déclaration d’interface dans ```windows.ui.xaml.hosting.desktopwindowxamlsource.h```.
...
Pour obtenir des exemples complets qui illustrent les tâches dans le contexte d’un exemple d’application de travail, voir les fichiers de code suivants:
  ...
  * **C/C++ Win32:** Consultez le fichier [Desktop.cpp](https://github.com/clarkezone/cppwinrt/blob/master/Desktop/XamlIslandsWin32/Desktop.cpp) dans l’exemple [XamlIslands32](https://github.com/clarkezone/cppwinrt/tree/master/Desktop/XamlIslandsWin32) sur GitHub.
  * **WPF:** Consultez les fichiers [WindowsXamlHostBase.cs](https://github.com/Microsoft/WindowsCommunityToolkit/blob/master/Microsoft.Toolkit.Win32/Microsoft.Toolkit.Wpf.UI.XamlHost/WindowsXamlHostBase.cs) et [WindowsXamlHost.cs](https://github.com/Microsoft/WindowsCommunityToolkit/tree/master/Microsoft.Toolkit.Win32/Microsoft.Windows.Interop.WindowsXamlHost.WPF/WindowsXamlHost.cs) dans le Kit de ressources de la Communauté Windows.  
...
3. Intégrer le code source pour le contrôle XAML UWP personnalisé à votre solution d’application hôte, générer le contrôle personnalisé et l’utiliser dans votre application en suivant [les instructions ci-après](https://github.com/Microsoft/WindowsCommunityToolkit/blob/master/docs/controls/WindowsXAMLHost.md#add-a-custom-uwp-control).
...
1. Lorsque l’élément d’interface utilisateur parent doit obtenir la taille de la zone rectangulaire nécessaire pour s’adapter à la **Windows.UI.Xaml.UIElement** que vous hébergez sur le **DesktopWindowXamlSource**, appelez la méthode de [**mesure**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.measure) de la **Windows.UI.Xaml.UIElement **. Exemple :
    * Dans une application WPF vous pourrez le faire à partir de la méthode [**MeasureOverride**](https://docs.microsoft.com/dotnet/api/system.windows.frameworkelement.measureoverride) de l' [**HwndHost**](https://docs.microsoft.com/dotnet/api/system.windows.interop.hwndhost.aspx) qui héberge le **DesktopWindowXamlSource**.
...
2. Lorsque la taille des modifications d’élément de l’interface utilisateur parent, appelez la méthode de [**disposition**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.arrange) de la racine **Windows.UI.Xaml.UIElement** qui vous hébergez sur le **DesktopWindowXamlSource**. Exemple :
    * Dans une application WPF vous pourrez le faire à partir de la méthode [**ArrangeOverride**](https://docs.microsoft.com/dotnet/api/system.windows.frameworkelement.arrangeoverride) de l’objet [**HwndHost**](https://docs.microsoft.com/dotnet/api/system.windows.interop.hwndhost.aspx) qui héberge le **DesktopWindowXamlSource**.
...
2. Dans votre fonction [**WindowProc**](https://msdn.microsoft.com/library/windows/desktop/ms633573.aspx) , écoutez le message [**WM_DPICHANGED**](https://docs.microsoft.com/windows/desktop/hidpi/wm-dpichanged) . En réponse à ce message:
    * Utilisez la fonction [**SetWindowPos**](https://msdn.microsoft.com/library/windows/desktop/ms633545) pour redimensionner la fenêtre qui contient le contrôle UWP pour le rectangle passé au message.
