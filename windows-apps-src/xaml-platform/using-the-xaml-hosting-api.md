---
author: mcleans
description: Cet article décrit comment héberger l’interface utilisateur de XAML UWP dans votre application de bureau.
title: À l’aide des API d’hébergement dans une application de bureau UWP XAML
ms.author: mcleans
ms.date: 09/21/2018
ms.topic: article
ms.prod: windows
...
ms.locfileid: "4532352"
---
# <a name="using-the-uwp-xaml-hosting-api-in-a-desktop-application"></a>À l’aide des API d’hébergement dans une application de bureau UWP XAML
...
* Si vous disposez d’une application de bureau C++ Win32 et que vous souhaitez pour héberger des contrôles UWP dans votre application, vous devez utiliser l’API d’hébergement de XAML UWP. Il n’existe aucune alternative pour ces types d’applications.
...
Il existe plusieurs exemples qui montrent comment utiliser le XAML UWP API d’hébergement dans une application C++ Win32 sur GitHub:
...
3. Intégrer le code source pour le contrôle XAML UWP personnalisé de votre solution d’application hôte, le contrôle personnalisé de génération et son utilisation dans votre application en suivant [les instructions ci-après](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/windowsxamlhost#add-a-custom-uwp-control).
...
L’API d’hébergement de XAML partage les mêmes limites que tous les autres types de contrôles d’hôte XAML pour Windows 10. Pour obtenir la liste détaillée, voir les [limites du contrôle hôte XAML](xaml-host-controls.md#limitations).
...
### <a name="error-attaching-to-a-window-on-a-different-thread"></a>Erreur d’association d’une fenêtre sur un thread différent
...
### <a name="error-attaching-to-a-window-on-a-different-top-level-window"></a>Erreur d’association d’une fenêtre sur une autre fenêtre de niveau supérieur
