...
La plateforme Windows universelle (UWP) prend en charge pour une utilisation pour le client et pour le serveur des WebSockets. L'espace de noms [**Windows.Networking.Sockets**](/uwp/api/windows.networking.sockets?branch=live) définit deux classes WebSocket pour une utilisateur par le client&mdash;[**MessageWebSocket**](/uwp/api/windows.networking.sockets.messagewebsocket?branch=live) et [**StreamWebSocket**](/uwp/api/windows.networking.sockets.streamwebsocket?branch=live). Voici la comparaison de ces deux classes WebSocket.
...
[**MessageWebSocket**](/uwp/api/windows.networking.sockets.messagewebsocket?branch=live) permet à un message WebSocket d'être lu/écrit dans une même opération. Par conséquent, il convient lorsque les messages ne sont pas très volumineux. La classe prend en charge à la fois UTF-8 et les messages binaires.
...
[**StreamWebSocket**](/uwp/api/windows.networking.sockets.streamwebsocket?branch=live) permet la lecture de sections d’un message avec chaque opération de lecture. Par conséquent, il convient lorsque des fichiers volumineux (tels que des photos ou vidéos) sont transférés. La classe prend en charge uniquement les messages binaires.
...
Utilisez la propriété [**StreamWebSocket.InputStream**](https://docs.microsoft.com/en-us/uwp/api/Windows.Networking.Sockets.StreamWebSocket.InputStream) et un élément [**DataReader**](/uwp/api/windows.storage.streams.datareader?branch=live) pour lire les données.
...
[**MessageWebSocket**](/uwp/api/windows.networking.sockets.messagewebsocket?branch=live) et [**StreamWebSocket**](/uwp/api/windows.networking.sockets.streamwebsocket?branch=live) ont une classe correspondante qui fournit des informations supplémentaires sur l'objet.

[**MessageWebSocketInformation**](/uwp/api/windows.networking.sockets.messagewebsocketinformation?branch=live)Fournit des informations sur un **MessageWebSocket**, et vous récupérez une instance de cette classe à l’aide de la propriété [**MessageWebSocket.Information**](/uwp/api/windows.networking.sockets.messagewebsocket.Information).

[**StreamWebSocketInformation**](/uwp/api/Windows.Networking.Sockets.StreamWebSocketInformation?branch=live)Fournit des informations sur un **StreamWebSocket**, et vous récupérez une instance de cette classe à l’aide de la propriété [**StreamWebSocket.Information**](/uwp/api/Windows.Networking.Sockets.StreamWebSocket.Information).
