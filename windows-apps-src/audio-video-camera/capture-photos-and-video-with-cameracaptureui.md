...
> [!NOTE]
> Vous ne devez pas spécifier les fonctionnalités de la **webcam** ou **le microphone** dans votre fichier manifeste d’application si votre application utilise uniquement CameraCaptureUI. Sinon, votre application sera affichée dans les paramètres de confidentialité de l’appareil photo, mais même si l’utilisateur n’autorise pas l’appareil photo à accéder à votre application, CameraCaptureUI pourra capturer du contenu multimédia. Cela s’explique par le fait que l’application d’appareil photo intégrée de Windows est une application interne approuvée qui nécessite que l’utilisateur démarre la capture photo, vidéo ou audio en appuyant sur un bouton. Votre application peut échouer certification Kit de Certification des applications Windows lorsqu’il est envoyé au magasin si vous spécifiez les fonctionnalités de la webcam ou le microphone lorsque vous utilisez CameraCaptureUI comme votre seul mécanisme de capture photo.
