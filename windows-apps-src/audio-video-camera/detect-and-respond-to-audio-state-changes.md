...
Dans le **SoundLevelChanged** Gestionnaire d’événements, vérifiez le [ **SoundLevel** ](https://docs.microsoft.com/uwp/api/windows.media.audio.audiostatemonitor.soundlevel) propriété de la **AudioStateMonitor** expéditeur passé dans le Gestionnaire pour déterminer ce qui est le nouveau niveau audio pour le flux. Dans cet exemple, l’application arrête la capture audio lorsque le niveau sonore est désactivé et reprend la capture lorsque le niveau sonore est restauré sur le volume complet.
