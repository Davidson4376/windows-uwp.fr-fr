...
À partir de Windows 10, version 1607, **AdvancedPhotoCapture** peut être utilisé pour capturer des photos à l’aide d’un algorithme intégré qui améliore la qualité des photos prises dans des conditions de faible luminosité.

> [!NOTE] 
> L’enregistrement simultané de capture photo et vidéo à l’aide de **AdvancedPhotoCapture** n’est pas pris en charge.

> [!NOTE] 
> Si la propriété [**FlashControl.Enabled**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Enabled) est définie sur true, elle remplace les paramètres **AdvancedPhotoCapture** et entraîne la capture avec flash d’une photo standard. Si la propriété [**Auto**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Auto) est définie sur true, la classe **AdvancedPhotoCapture** est utilisée telle que configurée et le flash n’est pas sollicité.

> [!NOTE] 
> Cet article repose sur les concepts et le code décrits dans [Capture photo, vidéo et audio de base à l’aide de MediaCapture](basic-photo-video-and-audio-capture-with-MediaCapture.md), qui décrit la procédure d’implémentation de la capture photo et vidéo de base. Nous vous recommandons de vous familiariser avec le modèle de capture simple de contenu multimédia de cet article avant d’adopter des scénarios de capture plus avancés. Le code de cet article repose sur l’hypothèse que votre application possède déjà une instance de MediaCapture initialisée correctement.
