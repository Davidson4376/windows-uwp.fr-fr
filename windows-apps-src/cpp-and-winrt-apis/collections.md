...
Vous pouvez éviter la surcharge liée à des appels à **Ajouter** dans l’exemple de code précédent. Vous disposez peut-être déjà la source de données, ou vous pouvez préférer remplir la source de données avant la création de l’objet de collection Windows Runtime. Voici la procédure à suivre.
...
Vous pouvez créer une collection Windows Runtime à partir des données et obtenir une présentation dessus prêt à transmettre à une API, sans quoi que ce soit copie.
...
Au lieu de cela, il est beaucoup plus facile à dériver de votre affichage vectoriel personnalisée à partir du modèle de structure [**winrt::vector_view_base**](/uwp/cpp-ref-for-winrt/vector-view-base) et implémenter simplement la fonction **get_container** pour exposer le conteneur contenant vos données.
