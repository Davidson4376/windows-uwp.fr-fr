...
Enregistrez et lancez la génération. Copiez `BookstoreViewModel.h` et `BookstoreViewModel.cpp` à partir du dossier `Generated Files` dans le dossier de projet, et incluez-les dans le projet. Ouvrez les fichiers et implémenter la classe d’exécution, comme indiqué ci-dessous. Note comment, dans `BookstoreViewModel.h`, nous allons notamment `BookSku.h`, qui déclare le type de mise en œuvre (**winrt::Bookstore::implementation::BookSku**). Et nous sommes restauration le constructeur par défaut en supprimant `= delete`.
