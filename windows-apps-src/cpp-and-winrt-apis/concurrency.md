...
Une coroutine est une fonction comme n’importe quel autre dans la mesure où un appelant est bloqué jusqu'à ce qu’une fonction retourne l’exécution à celui-ci. Et, en premier lieu la possibilité pour une coroutine renvoyer est le premier `co_await`, `co_return`, ou `co_yield`.
...
La raison pour laquelle vous pouvez compter sur ce comportement est étant donné que C++ / WinRT fournit le code pour adapter ces types d’opération asynchrone Windows Runtime à la coroutine langue prise en charge C++ (ces extraits de code sont appelés cartes d’attente). Les types restants await en C++ / WinRT sont simplement des wrappers de pool de threads et/ou helpers; Par conséquent, il effectuer sur le pool de threads.
...
Si vous `co_await` un autre type&mdash;même au sein de C++ / mise en œuvre de coroutine WinRT&mdash;ensuite, une autre bibliothèque fournit les adaptateurs, vous devez comprendre ce que ces cartes faire en termes de reprise et de contextes.

Pour conserver les changements de contexte jusqu’au minimum, vous pouvez utiliser certaines des techniques que nous avons déjà vu dans cette rubrique. Nous allons voir certaines illustrations de cette opération. Dans cet exemple de pseudo-code suivant, nous montrons le plan d’un gestionnaire d’événements qui appelle une API Windows Runtime pour charger une image, descend sur un thread d’arrière-plan pour traiter cette image, puis retourne au thread d’interface utilisateur pour afficher l’image dans l’interface utilisateur.
...
Pour ce scénario, il existe un peu d’ineffiency autour de l’appel à **StorageFile::OpenAsync**. Il existe un changement de contexte nécessaire pour un arrière-plan de threads (de sorte que le gestionnaire peut retourner l’exécution à l’appelant), lors de la reprise après les C + / WinRT restaure le contexte de thread d’interface utilisateur. Toutefois, dans ce cas, il n’est pas nécessaire de se trouver sur le thread d’interface utilisateur jusqu'à ce que nous sommes sur le point de mettre à jour l’interface utilisateur. Plus APIs Windows Runtime que nous appelons *avant* notre appel à **winrt::resume_background**, les commutateurs de contexte et basculant plus inutiles nous induire. La solution consiste à n’appeler *n’importe quel* APIs Windows Runtime avant. Les déplacer tous les après le **winrt::resume_background**.
...
Si vous souhaitez effectuer une action plus avancés, puis vous pouvez écrire votre propre await cartes. Par exemple, si vous souhaitez qu’un `co_await` reprise sur le même thread que l’action asynchrone se termine sur (il n’est donc aucun changement de contexte), puis vous pouvez commencer en écrivant await cartes semblables à ceux indiqués ci-dessous.

> [!NOTE]
> L’exemple de code ci-dessous est fournie uniquement; à des fins éducatives Il est de vous aider à comprendre await comment le travail de cartes. Si vous souhaitez utiliser cette technique dans votre propre code de base, puis nous recommandons que vous développez et testez votre propre await struct(s) de carte. Par exemple, vous pouvez écrire **complete_on_any**, **complete_on_current**et **complete_on(dispatcher)**. Envisagez également les rendre des modèles qui acceptent le type **IAsyncXxx** comme un paramètre de modèle.
...
Pour comprendre comment utiliser les **valeur_log** await adaptateurs, vous devez tout d’abord savoir que lorsque le compilateur C++ rencontre un `co_await` expression, il recherche des fonctions appelées **await_ready**, **await_suspend**et **await_resume**. C++ / WinRT bibliothèque fournit ces fonctions afin que vous obtenez comportement raisonnable par défaut, comme suit.
...
Puis, au lieu de recherche pour les trois fonctions **await_xxx** qui correspondent aux **IAsyncXxx**, le compilateur C++ se présente pour les fonctions qui correspondent aux **valeur_log**.

## <a name="canceling-an-asychronous-operation-and-cancellation-callbacks"></a>L’annulation d’une opération asynchrone et les rappels de l’annulation
...
Si vous exécutez l’exemple ci-dessus, puis vous verrez un message d’impression **ImplicitCancellationAsync** par seconde pour les trois secondes, après lequel fois automatiquement qu’il s’arrête à la suite en cours d’annulation. Cela fonctionne, car, en rencontrent un `co_await` expression, une coroutine vérifie si elle a été annulée. Si tel est le cas, puis elle court-circuite et si elle n’a pas encore, puis il suspend comme normal.
...
Dans cet exemple de code suivant, **NestedCoroutineAsync** effectue le travail, mais il n’a aucune raison d’annulation spécial qu’il contient. **CancellationPropagatorAsync** est essentiellement un wrapper sur la coroutine imbriqué; le wrapper transmet préalablement l’annulation.
