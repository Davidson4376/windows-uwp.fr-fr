...
Lvalue, à l’inverse, n’est pas mobile, comme illustré dans l’illustration suivante. Lvalue déplacée serait Relevez la définition de *lvalue*, et il serait un problème inattendu pour le code qui très raisonnablement s’attendre à être en mesure de continuer à accéder à la valeur lvalue.

![Rvalue est mobile; lvalue n’est pas](images/is-movable.png)

Vous ne pouvez pas transférer une valeur lvalue. Mais il *est* un type de glvalue (l’ensemble des choses avec l’identité) que vous pouvez déplacer&mdash;si vous savez ce que vous êtes en train de faire (y compris en veillant à ne pas y accéder après le déplacement)&mdash;et c’est la valeur de x. Nous y reviendrons cette idée une fois de plus ci-dessous, lorsque nous allons examiner l’image complète des catégories de valeur.
...
Rvalue est mobile. Une rvalue *référence* correspond toujours à rvalue (une valeur dont le contenu qu’il est supposé que nous n’avez pas besoin de conserver).

Toutefois, est une référence rvalue lui-même rvalue? Une référence rvalue *sans nom* (comme celles indiquées dans les exemples de code de valeur x ci-dessus) est une valeur x Oui, il est donc une valeur rvalue. Il préfère être liée à un paramètre de fonction de référence rvalue, telles que celle d’un constructeur de déplacement. À l’inverse (et éventuellement counter-intuitively), si une référence rvalue a un nom, l’expression consistant à ce nom est lvalue. Par conséquent, il *ne peut pas* être liée à un paramètre de référence rvalue. Mais il est facile afin de pouvoir faire&mdash;simplement un cast vers une référence de rvalue sans nom (une valeur x) à nouveau.
...
### <a name="im"></a>\!i\ & \!m
Le type de valeur qui n’a pas identité et n’est pas mobile est l’un combinaison que nous n’avons pas encore traité. Toutefois, nous pouvons ignorer, car cette catégorie n’est pas une idée utile dans le langage C++.
...
Cette section finale aurait des références rvalue, nous avons déjà décrit, avec le concept de différent d’une *référence de transfert*.
...
- Fait référence à la valeur rvalue raison (telles que `A&&`) existe est de sorte que vous pouvez créer une surcharge qui est optimisée pour le cas d’une variable temporaire (ou autre rvalue) passés.
