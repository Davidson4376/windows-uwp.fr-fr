---
description: Cette rubrique décrit les différentes catégories de valeurs qui existent en C++. Vous serez sans doute ont entendu lvalues et rvalues, mais il existe d’autres types, également.
title: Catégories de valeurs et des références associées
ms.date: 08/11/2018
ms.topic: article
keywords: Windows 10, uwp, standard, c++, cpp, winrt, projection, en déplaçant, transfert, catégories de valeurs, sémantique de déplacement, transfert parfait, lvalue, rvalue, glvalue, prvalue, valeur x
...
Une *lvalue* dispose d’identité. Il est désormais une question d’intérêt uniquement historique que le «l» en «lvalue» est l’abréviation de «gauche» (comme dans la gauche côté d’une affectation). En C++, lvalue peut s’afficher sur la gauche *ou* à droite d’une affectation. Le «l» en «lvalues», puis, ne prend pas réellement vous aident à comprendre, ni définir ce qu’ils sont. Vous devez uniquement comprendre que ce que nous appelons lvalue est une valeur qui a une identité.
...
Cette section présente la syntaxe pour une référence à une valeur rvalue. Nous allons devoir attendre une autre rubrique devienne un traitement substantiel de déplacement et de transfert, mais ce sont des problèmes qui sont résolus par des références rvalue. Avant de nous allons examiner les références rvalue, cependant, nous devons d’abord être réorganisées sur `T&` &mdash;la chose que nous avons précédemment été appelant simplement «une référence». Il s’agit vraiment «une lvalue (non const) référence», ce qui fait référence à une valeur sur laquelle l’utilisateur de la référence peut écrire.
...
La syntaxe pour une référence à une valeur rvalue de type `T` est écrit en tant que `T&&`. Une référence rvalue fait référence à une valeur mobile&mdash;une valeur dont nous n’avez pas besoin de conserver une fois que nous avons utilisé (par exemple, une variable temporaire) le contenu. Depuis le point entière est passer du (ainsi modification) la valeur liée à une référence rvalue, `const` et `volatile` qualificateurs (également connue sous le nom VC-qualificateurs) ne s’appliquent pas aux références rvalue.
...
Une valeur x (un type de glvalue, mais également un type de rvalue) a une identité et peut également être déplacé. Cela peut être lvalue premier que vous avez décidé de déplacer dans la mesure où la copie est coûteuse, et vous serez en veillant à ne pas y accéder par la suite. Voici comment vous pouvez transformer lvalue en une valeur x.
...
Plusieurs références similaires dans une expression (une référence lvalue à une référence lvalue, ou une référence rvalue à une référence rvalue) annuler l’une avec un autre en puissance parallèle.
...
Cette section finale aurait des références rvalue, nous avons déjà décrit, avec le concept d’une *référence de transfert*différents.
...
- Fait référence à la valeur rvalue raison (telles que `A&&`) existe est afin que vous pouvez créer une surcharge qui est optimisée pour le cas d’une variable temporaire (ou d’autre rvalue) passés.
...
- Le passage d’une valeur lvalue entraîne la référence de transfert à devenir `_Ty& &&`, ce qui réduit à la référence lvalue `_Ty&`.
- Le passage d’une valeur rvalue entraîne la référence de transfert à devenir `_Ty&& &&`, ce qui réduit à la référence rvalue `_Ty&&`.
