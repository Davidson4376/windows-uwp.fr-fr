...
Les informations figurant dans cette rubrique sont encadrée en termes d’analyse de Stroustrup de catégories de valeurs par les deux propriétés indépendantes d’identité et movability [Stroustrup, 2013].

## <a name="an-lvalue-has-identity"></a>Lvalue possède une identité
Que signifie pour une valeur *d’identité*? Si vous avez (ou vous pouvez tirer) à l’adresse de mémoire d’une valeur et l’utiliser en toute sécurité, puis la valeur a identité. De cette façon, vous pouvez faire plus que comparer le contenu de valeurs: vous pouvez comparer ou de les distinguer par identité.
...
Vous ne pouvez pas transférer lvalue. Mais il *est* un type de glvalue (l’ensemble des actions identité) que vous pouvez déplacer&mdash;si vous savez ce que vous êtes en train de faire (y compris en veillant à ne pas y accéder après le déplacement)&mdash;et c’est la valeur x. Nous y reviendrons cette idée une fois supplémentaire ci-dessous, lorsque nous allons examiner l’image complète des catégories de valeurs.
...
Une référence lvalue peut établir une liaison à une valeur lvalue, mais pas vers rvalue.
...
### <a name="im"></a>\!i\ & \!m
Le type de valeur qui n’a pas identité et n’est pas mobile est l’one combinaison que nous n’avons pas encore traité. Toutefois, nous pouvons ignorer, car cette catégorie n’est pas une idée utile dans le langage C++.
...
Cette section finale aurait des références rvalue, nous avons déjà indiqué, avec le concept d’une *référence de transfert*différents.
...
- Fait référence à la valeur rvalue raison (telles que `A&&`) existe est afin que vous pouvez créer une surcharge qui est optimisée pour le cas d’une variable temporaire (ou d’autres rvalue) passés.
