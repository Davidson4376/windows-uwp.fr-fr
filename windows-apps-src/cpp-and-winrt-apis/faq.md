...
Nous recommandons certainement à l’aide de const dans la mesure du possible, mais le `cppwinrt.exe` outil lui-même ne tente pas de raison pour laquelle sur l’implémentation de fonctions peut-être intentionnellement const, et qui ne peut pas. Vous pouvez choisir de rendre une de vos fonctions de mise en œuvre const, comme dans cet exemple.
...
Vous pouvez supprimer ces `const` qualificateur sur **ToString** doit vous décidez que vous deviez modifier certains état de l’objet dans son implémentation. Mais chacune de vos membre fonctions const ou non const, pas les deux. En d’autres termes, ne surchargez pas une fonction d’implémentation sur `const`.
...
> [!NOTE]
> Si cette rubrique n’a pas répondu à votre question, il est possible aide en consultant la [Communauté de développeurs Visual Studio C++](https://developercommunity.visualstudio.com/spaces/62/index.html)ou en utilisant le [ `c++-winrt` balise sur Stack Overflow](https://stackoverflow.com/questions/tagged/c%2b%2b-winrt).
