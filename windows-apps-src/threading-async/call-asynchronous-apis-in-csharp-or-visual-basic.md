...
Dans cette rubrique, nous n’allons pas expliquer en détail la manière dont le compilateur utilise l’opérateur **await**. Nous allons plutôt nous attacher à comprendre comment votre application peut rester asynchrone et réactive. Observons ce qui se produit avec du code synchrone. Par exemple, imaginons qu’il y ait une méthode synchrone appelée `SyndicationClient.RetrieveFeed`. (Il n’existe pas de méthode de ce type, mais imaginez qu’il y en a.) Si votre application incluait la ligne `SyndicationFeed feed = client.RetrieveFeed(feedUri)`, au lieu de `SyndicationFeed feed = await client.RetrieveFeedAsync(feedUri)`, l’exécution de l’application s’arrêtera jusqu’à ce que la valeur de retour de `RetrieveFeed` soit disponible. Pendant que l’application attend la fin de la méthode, elle ne peut plus répondre à d’autres événements (un deuxième événement [**Click**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click), par exemple). En clair, votre application resterait bloquée jusqu’au renvoi du résultat de `RetrieveFeed`.
...
**Conseil**  Starting avec C# dans Microsoft Visual Studio 2005, vous pouvez utiliser **await** dans le bloc **catch** .
