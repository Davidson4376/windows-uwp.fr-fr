...
Cet article fournit des indications de base sur la façon d’utiliser la classe **task** avec les API asynchrones UWP. Pour plus d’informations de documentation complète sur **tâche** et ses méthodes connexes, y compris [ **créer\_tâche**][createTask], consultez [ Parallélisme (Concurrency Runtime) des tâches][taskParallelism]. 
