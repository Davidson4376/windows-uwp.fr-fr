...
Vous pouvez accéder au bouton en toute sécurité à l’aide de sa propriété [Répartiteur](https://go.microsoft.com/fwlink/p/?LinkId=256026) ou de la propriété `Dispatcher` d’un objet existant dans le contexte de thread d’interface utilisateur (tel que la page où se trouve le bouton). Le code suivant utilise l'objet [CoreDispatcher](https://go.microsoft.com/fwlink/p/?LinkId=256029) de la méthode [RunAsync](https://go.microsoft.com/fwlink/p/?LinkId=256030) pour envoyer l’appel sur le thread d’interface utilisateur.
