...
Pour afficher des publicités dans vos applications Windows, installez l'un des kits de développement logiciel suivants:

* Pour les applications UWP pour Windows10, installez le [SDK Microsoft Advertising](http://aka.ms/ads-sdk-uwp). Ce kit de développement logiciel(SDK) est une extension de VisualStudio2015 et des versions ultérieures.
* Pour les applicationsWindows8.1 et WindowsPhone8.x, installez le [SDK Microsoft Advertising pour Windows et Windows Phone8.x](http://aka.ms/store-8-sdk). Ce Kit de développement logiciel (SDK) est une extension de VisualStudio2015 et de Visual Studio2013.

> [!NOTE]
> Si vous développez une application UWP en JavaScript/HTML et que vous avez installé le SDK Windows10 (14393) ou version ultérieure, vous devez également installer la bibliothèque WinJS. Cette bibliothèque était incluse dans les versions précédentes du SDK Windows10, mais à partir du SDK Windows10 (14393), vous devez l’installer séparément. Pour installer WinJS, voir [Obtenir WinJS](http://try.buildwinjs.com/download/GetWinJS/).
...
3.  Téléchargez et installez le [SDK Microsoft Advertising](http://aka.ms/ads-sdk-uwp) (pour les applications UWP pour Windows10) ou le [SDK Microsoft Advertising pour Windows et Windows Phone8.x](http://aka.ms/store-8-sdk) (pour les applications XAML et JavaScript/HTML pour Windows8.1 and Windows Phone8.x). L’installation peut prendre quelques minutes. Attendez la fin du processus.
...
## <a name="install-via-nuget-uwp-only"></a>Installer via NuGet (UWP uniquement)
...
3. Dans **Gestionnaire de références**, sélectionnez l’une des références suivantes en fonction de votre type de projet:

    -   Pour un projet de plateforme Windows universelle (UWP): développez **Windows universel**, cliquez sur **Extensions**, puis activez la case à cocher **SDK Microsoft Advertising pour XAML** (pour les applications XAML) ou **SDK Microsoft Advertising pour JavaScript** (pour les applications créées à l’aide de JavaScript et HTML).

    -   Pour un projet Windows8.1: développez **Windows8.1**, cliquez sur **Extensions**, puis activez la case à cocher **Kit de développement logiciel (SDK) Ad Mediator pour Windows8.1XAML** (pour les applications XAML) ou **SDK Microsoft Advertising pour Windows8.1 natif (JS)** (pour les applications créées à l’aide de JavaScript et HTML).

    -   Pour un projet Windows Phone8.1: développez **Windows Phone8.1**, cliquez sur **Extensions**, puis activez la case à cocher **** (pour les applications XAML) ou **SDK Microsoft Advertising pour Windows Phone8.1 natif (JS)** (pour les applications créées à l’aide de JavaScript et HTML).
...
<span id="framework" />
## <a name="understanding-framework-packages-in-the-microsoft-advertising-sdk-uwp-only"></a>Comprendre les packages d’infrastructure du SDK Microsoft Advertising (UWP uniquement)
