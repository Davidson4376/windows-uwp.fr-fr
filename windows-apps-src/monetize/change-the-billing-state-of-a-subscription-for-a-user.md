...
description: "Utilisez cette méthode dans l’API d’achat du WindowsStore afin de modifier l’état de facturation de l’abonnement d’un utilisateur."
...
ms.prod: windows
ms.technology: uwp
keywords: "windows10, uwp, API d’achat du WindowsStore, abonnements"
...
Utilisez cette méthode dans l’API d’achat du WindowsStore afin de modifier l’état de facturation de l’extension de l’abonnement d’un utilisateur donné. Vous pouvez annuler, étendre, rembourser ou désactiver le renouvellement automatique d'un abonnement.
...
* Une clé d’ID du WindowsStore représentant l’identité de l’utilisateur qui possède des droits à l’abonnement que vous souhaitez modifier.
...
|----------------|--------|---------------|----------|
| b2bKey         | chaîne | La [clé d’ID du WindowsStore](view-and-grant-products-from-a-service.md#step-4) qui représente l’identité de l’utilisateur dont vous souhaitez modifier l’abonnement.     | Oui      |
...
L’exemple suivant présente comment utiliser cette méthode afin d’étendre la période d’abonnement de 5jours. Remplacez la valeur *b2bKey* par la [clé d’ID du WindowsStore](view-and-grant-products-from-a-service.md#step-4) qui représente l’identité de l’utilisateur dont vous souhaitez modifier l’abonnement.
...
| productId | chaîne | [L’ID Store](in-app-purchases-and-trials.md#store-ids) pour le [produit](in-app-purchases-and-trials.md#products-skus-and-availabilities) qui représente l’extension d’abonnement dans le catalogue WindowsStore. Exemple d’ID Store pour un produit: 9NBLGGH42CFD.     |
| skuId | chaîne |  [L’ID Store](in-app-purchases-and-trials.md#store-ids) pour la [SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities) qui représente l’extension d’abonnement dans le catalogue WindowsStore. Exemple d’ID Store pour une SKU: 0010.    |
