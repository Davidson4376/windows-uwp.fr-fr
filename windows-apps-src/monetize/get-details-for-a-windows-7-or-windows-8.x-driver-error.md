...
Utilisez cette méthode dans l’API d’analyse du MicrosoftStore pour obtenir les informations concernant une erreur de pilote Windows 7 ou Windows 8.x spécifique de votre app, au format JSON. Pour utiliser cette méthode, vous devez d’abord utiliser la méthode [Obtenir les données de rapport d’erreurs pour les pilotes Windows 7 et Windows 8.x](get-error-reporting-data-for-windows-7-and-windows-8.x-drivers.md) afin de récupérer l’ID de l’erreur sur laquelle vous souhaitez des informations détaillées.
