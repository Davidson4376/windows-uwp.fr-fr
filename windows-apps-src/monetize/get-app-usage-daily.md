...
description: Utilisez cette méthode dans l’API d’analytique Microsoft Store pour obtenir des données d’utilisation quotidienne application pour une plage de dates donné et d’autres filtres facultatifs.
...
|------------|--------|-------------------------------------------------------------------------------------------------------------------------------------|
| Valeur      | tableau  | Tableau d’objets qui contiennent des données d’utilisation d’agrégation. Pour plus d’informations sur les données incluses dans chaque objet, voir le tableau suivant. |
...
| Valeur                     | Type    | Description                                                               |
|---------------------------|---------|---------------------------------------------------------------------------|
| date                      | chaîne  | La première date dans la plage de dates pour les données d’utilisation. Si la requête spécifiait un jour précis, cette valeur correspond à la date. Si la requête était relative à une semaine, un mois ou toute autre plage de dates, cette valeur correspond à la première date de la plage de dates.        |
| applicationId             | chaîne  | L’ID de Store de l’application pour laquelle vous récupérez des données d’utilisation.          |
| applicationName           | chaîne  | Nom d’affichage de l’application.                                              |
| deviceType                | chaîne  | Une des chaînes suivantes qui spécifie le type d’appareil où l’utilisation s’est produite :<ul><li>**PC**</li><li>**Téléphone**</li><li>**Console**</li><li>**Tablet PC**</li><li>**IoT**</li><li>**Serveur**</li><li>**HOLOGRAPHIQUE**</li><li>**Inconnu**</li></ul>                                                                                                         |
| packageVersion            | chaîne  | La version du package où l’utilisation s’est produite.                          |
| market                    | chaîne  | Le code de pays ISO 3166 du marché où le client a utilisé votre application. |
| subscriptionName          | chaîne  | Indique si l’utilisation par le biais de Xbox Game Pass.                            |
| dailySessionCount         | longue    | Le nombre de sessions utilisateur ce jour-là.                                  |
| engagementDurationMinutes | double  | Les minutes où les utilisateurs sont activement à l’aide de votre application mesurée par une période distincte, en commençant au lancement de l’application (démarrage du processus) et de fin qui se termine (fin du processus) ou après une période d’inactivité.             |
| dailyActiveUsers          | longue    | Le nombre de clients à l’aide de l’application ce jour-là.                           |
| dailyActiveDevices        | longue    | Le nombre d’appareils quotidiennes utilisée pour interagir avec votre application par tous les utilisateurs.  |
| dailyNewUsers             | longue    | Le nombre de clients ayant utilisé votre application pour la première fois ce jour-là.    |
| monthlyActiveUsers        | longue    | Le nombre de clients à l’aide de l’application du mois en question.                         |
| monthlyActiveDevices      | longue    | Le nombre d’appareils exécutant votre application pendant une période distincte entre le moment quand l’application démarre (processus de démarrage) et de fin qui se termine (fin du processus) ou après une période d’inactivité.                                      |
| monthlyNewUsers           | longue    | Le nombre de clients ayant utilisé votre application pour la première fois le mois en question.  |
...
* [Obtenir mensuel ussage d’application](get-app-usage-monthly.md)
