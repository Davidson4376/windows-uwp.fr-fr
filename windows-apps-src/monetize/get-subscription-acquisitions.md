...
|------------|--------|------------------|
| Value      | tableau  | Tableau d’objets qui contiennent des données d’agrégation abonnement complémentaire acquisition. Pour plus d’informations sur les données de chaque objet, consultez le [valeurs d’acquisition des abonnements](#subscription-acquisition-values) section ci-dessous.             |
