...
|---------------|--------|---------------|------|
| applicationId | chaîne | Le *productId* du jeu Xbox One pour lequel vous récupérez des données d’acquisition. Pour obtenir l' *ID du produit* de votre jeu, accédez à votre jeu dans le programme d’Analytique XDP et récupérer l' *ID du produit* à partir de l’URL. Par ailleurs, si vous téléchargez vos données d’acquisitions à partir du rapport analytique de l’espace partenaires, l' *ID du produit* est inclus dans le fichier .tsv. |  Oui  |
