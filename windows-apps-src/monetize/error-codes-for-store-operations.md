---
description: Cet article décrit les codes d’erreur courants pour les opérations du Windows Store pour les applications et modules complémentaires, y compris des achats dans l’application, les licences et installer des mises à jour.
...
| 0x803F6102   | L’extension spécifiée est introuvable. L’extension n’est plus peut-être être disponible dans le Windows Store ou votre peut avoir fourni le mauvais ID Windows Store pour l’extension.                                               |
...
| 0x803F610C   | L’application ne prend pas en charge le contrat de tâche en arrière-plan spécifiée.                                             |
| 0x80040001   | La liste fournie de produit complémentaire ID n’est pas valide.                        |
...
| 0x803F9006    |  L’opération n’a pas pu aboutir, car l’utilisateur qui est autorisé à ce produit n'est pas connecté à l’appareil avec leur compte Microsoft.            |
...
## <a name="self-install-update-error-codes"></a>Installer des codes d’erreur de mise à jour automatique
