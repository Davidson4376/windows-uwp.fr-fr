...
## <a name="what-is-a-loose-file-layout"></a>Qu’est une disposition de fichiers libres?
...
> [!WARNING]
> Inscription de disposition de fichiers libres est destiné aux développeurs et aux concepteurs de valider rapidement leurs applications au cours du développement active. Cette approche ne doit pas être utilisé pour «dogfood» ou l’application de la version d’évaluation. Nous recommandons que la validation finale être exécutée sur une application empaquetée qui est signée avec un certificat approuvé. 
...
- Vos périphériques doivent être sur Windows 10 Creators Update (Build 14965) ou une version ultérieure.
...
1. Entrez le chemin d’accès du partage réseau vers la disposition de fichier isolé. 
...
L’appareil sur lequel l’inscription se déroule devront avoir accès à la disposition de fichier. Si le schéma du fichier est hébergé sur un partage réseau, assurez-vous que l’appareil a accès. 
