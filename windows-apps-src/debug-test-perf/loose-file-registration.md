...
## <a name="how-to-register-a-loose-file-layout"></a>Comment s’inscrire une disposition de fichiers isolés
...
Si vous utilisez les outils du SDK correspondant vers Windows 10 Creators Update (Build 14965) ou version ultérieure, vous pouvez utiliser le `WinDeployAppCmd` commande dans une invite de commandes.
...
Windows PowerShell vous permet également d’inscrire des dispositions de fichiers libres, mais uniquement sur l’appareil local. Si vous avez besoin inscrire une disposition à un appareil distant, vous devez utiliser l’une des autres méthodes. 

Pour inscrire la disposition de fichier isolé, lancement PowerShell et entrez les informations suivantes.
...
Actuellement, les lecteurs réseau mappés ne sont pas pris en charge pour les enregistrements de fichiers isolés. Reportez-vous au lecteur mappé avec full le chemin d’accès du partage réseau.
