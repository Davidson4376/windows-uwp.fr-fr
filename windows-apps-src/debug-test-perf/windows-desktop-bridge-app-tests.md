...
description: Utiliser des tests intégrés du pont du bureau pour vous assurer que votre application de bureau est optimisée pour sa conversion en une application UWP.
ms.date: 12/18/2017
ms.topic: article
...
[Applications de pont du bureau](https://docs.microsoft.com/en-us/windows/uwp/porting/desktop-to-uwp-root) sont des applications de bureau est converties en applications Universal Windows Platform (UWP) à l’aide de Windows le [pont du bureau](https://developer.microsoft.com/en-us/windows/bridges/desktop). Après la conversion, les applications de bureau Windows sont empaquetées, soumises à maintenance et déployées sous la forme d’un package d’application UWP (fichier .appx ou .appxbundle) ciblant Windows 10 Desktop.

## <a name="required-versus-optional-tests"></a>Tests obligatoires et tests facultatifs
Tests facultatifs pour les applications de pont du bureau Windows sont d’information uniquement et ne seront pas utilisées pour évaluer votre application pendant l’intégration de Microsoft Store. Nous vous recommandons d’examiner les résultats pour produire de meilleures applications de qualité des tests. Les critères généraux de réussite/échec d’intégration au Windows Store sont déterminés par les tests obligatoires et non par ces tests facultatifs.

## <a name="current-optional-tests"></a>Tests facultatifs actuels

### <a name="1-digitally-signed-file-test"></a>1. Signé numériquement le test de fichier 
**Arrière-plan**  
...
**Actions correctives**  
Consultez [bureau pour UWP Bridge : Extensions d’application](https://docs.microsoft.com/en-us/windows/uwp/porting/desktop-to-uwp-extensions) pour une explication de ces extensions et comment les utiliser correctement. 
...
### <a name="4-package-sanity-test"></a>4. Test de tranquillité d’esprit de package
...
### <a name="1-app-capabilities-test-special-use-capabilities"></a>1. Test de fonctionnalités d’application (utilisation spéciale de fonctionnalités)
...
### <a name="2-app-manifest-resources-tests"></a>2. Tests de ressources le manifeste d’application 
...
**Détails du test**  
Examine le manifeste de l’application afin de vérifier que son contenu est correct, comme décrit dans [Exigences relatives aux packages d’applications](https://docs.microsoft.com/en-us/windows/uwp/publish/app-package-requirements). Les vérifications suivantes sont effectuées au cours de ce test :
...
* Vérifie que chaque fichier binaire managé dans le package d’application n’a pas une dépendance sur une API Win32 qui n’est pas pris en charge pour le développement d’applications UWP en vérifiant la table d’adresse importation du fichier binaire.
...
**Détails du test**  
Une application ne peut pas demander d’élévation d’administrateur ou UIAccess conformément à la stratégie de Microsoft Store. Les autorisations de sécurité élevées ne sont pas prises en charge. 

**Actions correctives**  
Les applications doivent s’exécuter en tant qu’utilisateur interactif. Pour plus d’informations, voir [Vue d’ensemble de la sécurité UI Automation](https://go.microsoft.com/fwlink/?linkid=839440).
...
### <a name="8-windows-security-features-tests"></a>8. Tests de fonctionnalités de sécurité de Windows
...
**Actions correctives**  
Pour corriger ce problème, utilisez la dernière version de la [Bing Maps SDK](https://go.microsoft.com/fwlink/p/?linkid=614880) pour les applications UWP.
