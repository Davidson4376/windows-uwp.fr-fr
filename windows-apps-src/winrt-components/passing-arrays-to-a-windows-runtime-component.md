...
-   ReadOnlyArrayAttribute : Le tableau est copié quand l’appel franchit la limite (ABI) de l’interface binaire d’application. Les éléments sont convertis si nécessaire. Par conséquent, toute modification accidentelle que la méthode effectue dans un tableau d’entrée uniquement n’est pas visible pour l’appelant.
-   WriteOnlyArrayAttribute : La méthode appelée ne peut pas faire d’hypothèses concernant le contenu du tableau d’origine. Par exemple, le tableau que la méthode reçoit peut ne pas être initialisé ou peut contenir des valeurs par défaut. La méthode est censée définir les valeurs de tous les éléments du tableau.
