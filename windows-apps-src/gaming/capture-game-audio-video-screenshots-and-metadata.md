...
ms.date: 02/08/2017
ms.topic: article
keywords: windows 10, jeu, capturer, audio, vidéo, métadonnées
...
Les métadonnées que vous écrivez avec **AppCaptureMetadataWriter** sont mises en cache par le système jusqu’à ce qu’elles soient écrites dans le flux de média associé. Le système définit une limite de taille pour le cache de métadonnées de chaque application. Une fois la limite de taille du cache atteinte, le système commence à vider les métadonnées mises en cache. Le système supprime les métadonnées qui a été écrit avec **[AppCaptureMetadataPriority.Informational](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatapriority)** valeur de priorité avant de supprimer les métadonnées avec le **[ AppCaptureMetadataPriority.Important](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatapriority)** priorité.
...
Vous pouvez vous inscrire pour recevoir une notification lorsque le système commence à purge les métadonnées pour votre application en enregistrant un gestionnaire pour le **[MetadataPurged](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatawriter.MetadataPurged)** événement.
