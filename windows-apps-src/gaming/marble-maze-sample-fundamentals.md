...
# <a name="marble-maze-sample-fundamentals"></a>Principes de base de l’exemple Marble Maze
...
-   Utilisez le modèle **Application DirectX 11 (Windows universel)** dans une application C++ pour créer votre jeu UWP en DirectX. Utilisez Visual Studio pour générer un projet d’application pour UWP, comme vous le feriez pour générer un projet standard.
...
L’un des paramètres de projet importants du modèle **Application DirectX 11 (Windows universel)** est l’option **/ZW** qui permet au programme d’utiliser les extensions de langage Windows Runtime. Cette option est activée par défaut lorsque vous utilisez le modèle Visual Studio.
...
Après avoir déployé le projet, choisissez la vignette Marble Maze pour exécuter le jeu. Vous pouvez également choisir dans Visual Studio, dans la barre de menus, **Déboguer, Démarrer le débogage**.
