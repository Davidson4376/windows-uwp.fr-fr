...
description: Ce document décrit les caractéristiques fondamentales du projet Marbre labyrinthe; par exemple, comment elle utilise Visual C++ dans l’environnement d’exécution de Windows, comment elle est créée et structuré et comment elle repose.
...
-   Utilisez les [annotations SAL](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects) avec les outils d’analyse du code pour aider à découvrir des erreurs dans votre application.

## <a name="creating-the-visual-studio-project"></a>Création du projet Visual Studio


Si vous avez téléchargé et extrait l’exemple, vous pouvez ouvrir le fichier **MarbleMaze_VS2017.sln** (dans le dossier **C++** ) dans Visual Studio, et vous avez le code devant vous.
...
2. Dans la fenêtre **Nouveau projet** , dans le volet gauche, sélectionnez **Installed > Modèles > Visual C++**.

3. Dans la liste du milieu, sélectionnez **Application de 11 DirectX (Universal Windows)**. Si vous ne voyez pas cette option, vous deviez pas les composants requis installés&mdash;voir [Modifier Visual 2017 de Studio en ajoutant ou supprimant des charges de travail et des composants](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) pour plus d’informations sur l’installation des composants supplémentaires.
...
Vous pouvez utiliser tactile, l’accéléromètre, le contrôleur Xbox un ou contrôle Marbre labyrinthe de la souris.

-   Utilisez la croix directionnelle de la manette pour modifier l’élément de menu actif.
-   Utiliser les fonctions tactiles, les démarrer un bouton sur le contrôleur ou de la souris pour sélectionner un élément de menu.
-   Utilisez les fonctions tactiles, l’accéléromètre, le stick analogique gauche ou la souris pour incliner le jeu.
-   Utiliser les fonctions tactiles, les démarrer un bouton sur le contrôleur ou de la souris pour fermer les menus tels que la haute score table.
-   Utilisez le bouton Démarrer sur le contrôleur ou la touche P sur le clavier pour suspendre ou reprendre la partie.
-   Utilisez le bouton Retour de la manette ou la touche Début du clavier pour redémarrer le jeu.
-   Lorsque le tableau de score élevé est visible, utilisez le bouton précédent sur le contrôleur ou la touche origine sur le clavier pour effacer tous les résultats.

##  <a name="code-conventions"></a>Conventions de code


Windows Runtime est une interface de programmation permettant de créer des applications pour UWP qui ne peuvent être exécutées que dans un environnement d’application particulier. Ces applications utilisent des fonctions autorisées, les types de données et les périphériques et sont distribuées à partir de Microsoft Store. Au niveau le plus bas, Windows Runtime est composé d’une interface binaire d’application. L’interface binaire d’application est un contrat binaire de bas niveau qui rend les API Windows Runtime accessibles à plusieurs langages de programmation tels que les langages JavaScript, .NET et Visual C++.
...
Marble Maze utilise **^** et **Microsoft::WRL::ComPtr** pour gérer les objets alloués par segment de mémoire et limiter les fuites de mémoire. Nous vous conseillons d’utiliser ^ pour gérer la durée de vie des variables Windows Runtime, **ComPtr** pour gérer la durée de vie des variables COM (par exemple, lorsque vous utilisez DirectX) et **std::shared\_ptr** ou **std::unique\_ptr** pour gérer la durée de vie de tous les autres objets C++ segment de mémoire allouée.
...
-   Bien que nous vous recommandons de ne pas l’utilisation de **HRESULT** pour des erreurs inattendues, il est plus important afin d’éviter l’utilisation de la gestion des exceptions pour contrôler le flux de code. Utilisez plutôt une valeur de retour **HRESULT**, si nécessaire, pour contrôler le flux de code.
...
Prendre en compte la méthode **BasicLoader::LoadMesh** , qui est déclarée dans [BasicLoader.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/e62d68a85499e208d591d2caefbd9df62af86809/C%2B%2B/Shared/BasicLoader.h). Cette méthode utilise `_In_` pour spécifier le *nom de fichier* est un paramètre d’entrée (et par conséquent seront uniquement lues dans), `_Out_` pour spécifier que *vertexBuffer* et *indexBuffer* sont des paramètres de sortie (et par conséquent seront uniquement écrits dans) et `_Out_opt_` pour indiquer que *vertexCount* et *indexCount* facultatifs paramètres de sortie (et peuvent être écrites à). Étant donné que *vertexCount* et *indexCount* sont des paramètres de sortie optionnels, ils peuvent avoir la valeur **nullptr**. L’outil d’analyse du code C/C++ examine les appels à cette méthode pour s’assurer que les paramètres qu’elle transmet répondent à ces critères.
...
Pour effectuer l’analyse du code dans votre application, dans la barre de menus, choisissez **Créer > exécuter l’analyse du Code sur la Solution**. Pour plus d’informations sur l’analyse de code, voir [Analyse de la qualité du codeC/C++ à l’aide de l’analyse du code](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis).
