...
> **Remarque**    applications de bureau de Windows ont des API différentes pour obtenir une chaîne de permutation Direct3D, tel que [ **D3D11Device::CreateDeviceAndSwapChain**](https://msdn.microsoft.com/library/windows/desktop/ff476083)et n’utilisez pas un [ **CoreWindow** ](https://msdn.microsoft.com/library/windows/apps/br208225) objet.
...
Après avoir obtenu la référence [**CoreWindow**](https://msdn.microsoft.com/library/windows/apps/br208225), activez la fenêtre afin que celle-ci exécute la méthode **Run** de votre objet principal et commence le traitement de l’événement fenêtre. Après cela, créez un [ **ID3D11Device1** ](https://msdn.microsoft.com/library/windows/desktop/hh404575) et un [ **ID3D11DeviceContext1**](https://msdn.microsoft.com/library/windows/desktop/hh404598)et les utiliser pour obtenir sous-jacent [ **IDXGIDevice1** ](https://msdn.microsoft.com/library/windows/desktop/ff471331) et [ **IDXGIAdapter** ](https://msdn.microsoft.com/library/windows/desktop/bb174523) afin que vous pouvez obtenir un [ **IDXGIFactory2** ](https://msdn.microsoft.com/library/windows/desktop/hh404556) objet pour créer une ressource de chaîne de permutation selon votre [ **DXGI\_échange\_chaîne\_DESC1** ](https://msdn.microsoft.com/library/windows/desktop/hh404528) configuration.
...
Notez que Direct3D 11 ne propose pas d’abstraction identique à EGLSurface. (Il est [ **IDXGISurface1**](https://msdn.microsoft.com/library/windows/desktop/ff471343), mais elle est utilisée différemment.) L’approximation conceptuelle le plus proche est la [ **ID3D11RenderTargetView** ](https://msdn.microsoft.com/library/windows/desktop/ff476582) que nous utilisons pour attribuer une texture de l’objet ([**ID3D11Texture2D** ](https://msdn.microsoft.com/library/windows/desktop/ff476635)) en tant que la mémoire tampon d’arrière-plan qui dessine dans notre pipeline de nuanceur.
...
Dans Direct3D 11, créez une ressource [**ID3D11Texture2D**](https://msdn.microsoft.com/library/windows/desktop/ff476635), puis définissez-la en tant que cible de rendu. Configurer la cible de rendu à l’aide [ **D3D11\_restituer\_cible\_vue\_DESC**](https://msdn.microsoft.com/library/windows/desktop/ff476201). Lorsque vous appelez le [ **ID3D11DeviceContext::Draw** ](https://msdn.microsoft.com/library/windows/desktop/ff476407) (méthode) (ou un dessin similaire\* opération sur le contexte de périphérique) à l’aide de cette cible de rendu, les résultats sont dessinées dans une texture.
...
Dans Direct3D 11, vous configurez vos mémoires tampons et liez les nuanceurs à l’aide de la méthode [**IDXGISwapChain::Present1**](https://msdn.microsoft.com/library/windows/desktop/hh446797). Puis vous appelez une de la [ **ID3D11DeviceContext1::Draw** ](https://msdn.microsoft.com/library/windows/desktop/ff476407) \* méthodes pour exécuter les nuanceurs et dessiner les résultats à une cible de rendu configurée en tant que la mémoire tampon d’arrière-plan pour la chaîne de permutation. Enfin, vous présentez la mémoire tampon d’arrière-plan à l’affichage en appelant la méthode **IDXGISwapChain::Present1**.
...
| eglSurfaceAttrib                 | Utilisez [ **D3D11\_restituer\_cible\_vue\_DESC** ](https://msdn.microsoft.com/library/windows/desktop/ff476201) pour configurer une vue de cible de rendu Direct3D,                                                                                                                                                                                                                                                                                                                                                               |
