...
**Phase 2 : Utilisez MRT pour identifier et localiser des ressources**
...
Après avoir effectué les phases de 0 à 3 susmentionnées, vous disposerez d’un « lot » d’application, qui pourra être soumis au Microsoft Store et qui réduira les tailles de téléchargement et d’installation pour les utilisateurs, en omettant les ressources dont ces derniers n’ont pas besoin (par ex., les langues qu’ils ne parlent pas). Vous pouvez apporter des améliorations supplémentaires au niveau de la taille et des fonctionnalités de l’application en effectuant une étape finale. 

**Phase 4 : Migrer vers des formats de ressources MRT et API**
...
Si vous souhaitez utiliser le [Desktop App Converter](https://aka.ms/converter), reportez-vous à [le **bureau pour UWP Bridge : Desktop App Converter** sujet sur MSDN](https://aka.ms/converterdocs) pour plus d’informations sur le processus de conversion. Vous trouverez un ensemble complet d’exemples de convertisseur de bureau sur [le référentiel GitHub **Exemples de pont de bureau vers UWP**](https://github.com/Microsoft/DesktopBridgeToUWP-Samples).
...
### <a name="step-11-update-strings--assets-in-the-appxmanifest"></a>Étape 1.1 : Mettre à jour les chaînes de & ressources dans le fichier AppXManifest
...
0. Créer le `Strings\en-us` dossier (ou autre langage comme il convient) dans votre projet et ajoutez un **un nouvel élément** dans le dossier racine de votre projet, du nom par défaut de `resources.resw`
...
**Mise à jour le manifeste pour référencer les ressources**
...
*Remarque : le nombre d’éléments du manifeste ont été omis à partir de cet extrait de code, ne supprimez pas quoi que ce soit !*
...
### <a name="step-12-build-pri-file-make-an-appx-package-and-verify-its-working"></a>Étape 1.2 : Générer le fichier PRI, qu’un package AppX et vérifiez qu’il fonctionne
...
**Création et la signature du package**
 ...
 * `/o` définit pour remplacer le fichier de sortie s’il existe
0. Une fois le package créé, il doit être signé. Le moyen le plus simple pour obtenir un certificat de signature est en créant un projet Windows universel vide dans Visual Studio et en copiant le `.pfx` fichier qu’il crée, mais vous pouvez en créer un manuellement à l’aide de la `MakeCert` et `Pvk2Pfx` utilitaires comme décrit dans <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/jj835832.aspx"> Comment créer un package d’application certificat de signature</a>, une rubrique sur MSDN. 
 * **Important :** Si vous créez manuellement un certificat de signature, assurez-vous que vous placez les fichiers dans un autre répertoire que votre projet de code source ou de la source du package, sinon qu'il peut obtenir inclus dans le cadre du package, y compris la clé privée !
...
0. Acceptez l’invite d’élévation de contrôle de compte d’utilisateur administrateur, si elle s’affiche, puis cliquez sur `Next`
...
**Mettre à jour le manifeste AppX à la liste prise en charge des langues**
...
**Reconstruisez avec les valeurs localisées**
...
0. Accédez à `Time & language`
0. Accédez à `Region & language`
...
### <a name="step-14-localizing-more-parts-of-the-appx-manifest-optional"></a>Étape 1.4 : Localisation de plusieurs parties du manifeste AppX (facultatif)
...
**Disposition du fichier de ressources**
...
**Chargement des ressources d’interface utilisateur multilingue de Win32**
...
**À l’aide de l’outil de génération de l’offre groupée**
...
**Création manuelle des packages de ressources**
