...
-   [**Classe de Windows.Storage.KnownFolders**](https://docs.microsoft.com/uwp/api/Windows.Storage.KnownFolders)
...
-   **Déclarations Betacam d’application**
    ...
    > [!NOTE]
    > Contenu de la bibliothèque de Documents d’un groupe résidentiel n’est pas visible à votre application, quels que soient les fonctionnalités déclarées dans le manifeste de votre application et indépendamment des paramètres de partage de l’utilisateur.     

-   **Comprendre comment utiliser les sélecteurs de fichier**
...
-   **Comprendre les requêtes de fichier et dossier**
...
2.  **Afficher le sélecteur de fichiers et traiter le fichier sélectionnée.**
...
3.  **Exécutez la requête et traiter les résultats.**
...
1.  **Obtenir une collection d’utilisateurs du groupe résidentiel.**
...
2.  **Recherchez le dossier de l’utilisateur cible et ensuite créer une requête de fichier limitée à un dossier de l’utilisateur.**
...
1.  **Incluez un MediaElement dans votre application.**
...
2.  **Ouvrez un sélecteur de fichiers au groupe résidentiel et appliquer un filtre qui inclut des fichiers vidéo dans les formats pris en charge par votre application.**
...
3.  **Ouvrir la sélection de fichier de l’utilisateur pour un accès en lecture et définir le flux de fichier comme source pour le** [ **MediaElement**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.MediaElement), puis lancez le fichier.
    ```cs
    if (file != null)
    {
