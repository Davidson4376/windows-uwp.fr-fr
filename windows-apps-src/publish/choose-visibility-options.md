...
- Vous pourrez voir tous les avis rédigés par vos testeurs dans le [Rapport sur les avis](reviews-report.md). Toutefois, ces avis ne seront pas publiés dans la description de votre application dans le Store, même si votre soumission est ensuite définie sur **Public non privé**.  
- Lorsque vous modifiez une application de **Public privé** à **Public non privé**, la **date de publication** indiquée dans la description dans le Store correspond à la date de sa première publication pour un public non privé (sauf si vous avez spécifié une date différente pour [Afficher la date de publication](set-app-pricing-and-availability.md#display-release-date)).
