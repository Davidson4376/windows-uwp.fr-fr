...
> [!IMPORTANT]
> Lorsque le lancement progressif de packages dans une version d’évaluation, les personnes qui ne sont pas incluses dans le pourcentage et qui récupère vos nouveaux packages obtiendront les packages à partir de la soumission précédente (sauf si une version d’évaluation de niveau supérieur est disponible sur ces derniers).
