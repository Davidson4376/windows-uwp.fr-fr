...
Les comptes d’entreprise sont un peu plus onéreux, car des étapes supplémentaires sont nécessaires pour vérifier que vous êtes autorisé à configurer le compte au nom de votre entreprise. Selon les [Stratégie du Store](https://docs.microsoft.com/legal/windows/agreements/store-policies#1014-account-type), un compte d'entreprise est obligatoire pour publier des applications avec certaines fonctionnalités, notamment les applications ayant accès aux informations financières du compte ou nécessitant l'authentification pour accéder aux fonctionnalités principales (sans passer par un fournisseur d'authentification tiers dédié et sécurisé).
